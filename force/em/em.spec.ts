...
import { Atom } from "../../old/atom/atom.ts";
import type { Meta } from "../dsl/meta/metafor.t";
...
describe("Каналы коммуникации между атомами", () => {
  ...
  describe("История импульсов", () => {
    ...
    it("срез содержит патчи импульсов", () => {
      expect(
        lastChunk.length,
        "срез должен содержать патчи импульсов",
      ).toBeGreaterThan(0);
    });
    ...
    it("сохраняет патч контекста в истории", () => {
      const hasContextPatch = lastChunk.some(
        (impulse) => impulse.path === "/context",
      );
      expect(hasContextPatch, "контекст должен обновляться в истории").toBe(
        true,
      );
    });
  });
  ...
  describe.skip("Двойная отправка сообщений (BroadcastChannel + внутренний механизм)", () => {
    it.skip("должен отправлять сообщения в оба канала", async () => {
      ...
      const contextMessage = messages.find((photon) =>
        photon.impulses.some(
          (patch: any) => patch.op === "replace" && patch.path === "/context",
        ),
      );
