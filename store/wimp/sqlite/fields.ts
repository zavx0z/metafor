  ...
  constructor(
    ...
    const row = (
      await this.sql<Array<{ required: number }>>`
        SELECT required FROM field WHERE wimp = ${this.src} AND key = ${this.key}
      `
    )[0]
    ...
    const row = (
      await this.sql<Array<{ label: string | null }>>`
        SELECT label FROM field WHERE wimp = ${this.src} AND key = ${this.key}
      `
    )[0]
    if (!row) throw new Error(`field ${this.key} not found in meta ${this.src}`)
  ...
  async default(): Promise<string | undefined> {
    const row = (
      await this.sql<Array<{ default_value: string }>>`
        ...
        WHERE f.wimp = ${this.src} AND f.key = ${this.key}
      `
    )[0]
...
export class Fields {
  constructor(
    ...
    const rows = await this.sql<Array<{ key: string; type: FieldType }>>`
      SELECT key, type FROM field WHERE wimp = ${this.src} ORDER BY rowid
    `
    ...
    const row = (
      await this.sql<Array<{ type: FieldType }>>`
        SELECT type FROM field WHERE wimp = ${this.src} AND key = ${filter.key} LIMIT 1
      `
    )[0]
  ...
  async count(): Promise<number> {
    const row = (
      await this.sql<Array<{ count: number }>>`
        SELECT COUNT(*) AS count FROM field WHERE wimp = ${this.src}
      `
    )[0]
    return row?.count ?? 0
  }
  ...
  async exists(): Promise<boolean> {
    const row = (
      await this.sql<Array<{ ok: number }>>`
        SELECT 1 AS ok FROM field WHERE wimp = ${this.src} LIMIT 1
      `
    )[0]
