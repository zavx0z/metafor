...
Имя `Energy` освобождено для следующего слоя: распределённого исполнения
процессов, которое будет слушать photons Matrix, claim-ить process через `z` и
возвращать результат через `w+`/`w-`.
...
- [Онтология](docs/ONTOLOGY.md) фиксирует, что существует в MetaFor в доменах `Dark`, `Boundary`, `Matrix` и `Bulk`.
