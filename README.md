...
Каноническая логическая модель, математическая формализация бесконечно-конечного автомата, миссия и текущие runtime-инварианты поддерживаются в отдельном репозитории [`zavx0z/concept`](https://github.com/zavx0z/concept). Этот репозиторий является основной реализацией.
...
Внутри этой рамки текущий стартовый контур MetaFor разворачивается в пять рабочих доменных проекций:

- `Dark` как скрытая связность, скрытая организация, память и эволюция модели,
- `Boundary` как каноническая персистентная граница materialized мира,
- `Matrix` как вычислительный рантайм, где протекают переход, время и изменение состояния,
- `Energy` как исполнитель процессов с собственной рабочей mass,
- `Bulk` как проявленная объёмная композиция, наблюдаемая форма и пространственная принадлежность.

`Force` связывает эти проекции единым transport-ом. Dark передаёт declaration через поштучные Inflaton particles. Boundary атомарно фиксирует materialized изменение и испускает локальные последствия. Matrix передаёт process-bound State через Photon. Energy claim-ит Process через `z` и возвращает `w+`/`w-`.
...
`Weak` выполняется параллельно на WebGPU, когда GPU доступен. CPU сохраняется как детерминированный fallback и reference backend. Отдельного TypeScript-вычислителя State и второй Matrix-проекции не существует.

Одна обычная изменённая entity передаётся одним `ForceMessage` с одной `Particle`. Boundary остаётся единственной канонической materialized persistence; Matrix, Energy и Bulk не читают её SQLite напрямую и не становятся вторыми базами мира.

Для cold start и structural Meta-изменения Boundary может передать Matrix производную target-specific проекцию `runtime/matrix`, достаточную для сборки packed `gravity/strong/weak`. Это не второй canonical snapshot: проекция не владеет миром, может быть отброшена и полностью восстанавливается из Boundary. Запрещены именно параллельная durable truth, скрытый DB sync и operational snapshot, подменяющий причинный поток.

Первый non-visual runtime может работать без Bulk: все Impulse наблюдаются через структурированные server logs. Bulk с WebGPU и reusable UI остаются частями ядра и подключаются после стабилизации причинного цикла.
...
Каноническое ядро и математическая модель уже зафиксированы. В реализации восстановлен единый packed Matrix runtime, удалён параллельный evaluator, WebGPU возвращён как основной backend с CPU fallback, а Force получил структурированные Impulse logs. Следующая проверка - фактический CPU/WebGPU прогон, затем canonical путь `W result → Boundary/world update`, Reaction и подключение Bulk.

Проект не описывается как стабильная production-платформа. Текущая задача - получить минимальный воспроизводимый полный причинный цикл и уже на нём продолжать развитие среды.

## Быстрый запуск ядра

Минимальный runtime без Bulk и interpreter:

```bash
bun run runtime
```

С полным журналом Impulse:

```bash
bun run runtime:logs
```

Явный CPU fallback и строгий WebGPU режим:
...
Дальнейшая разработка ядра ведётся локально через Codex app, обычные Git branches/PR, terminal commands и tests. Interpreter сохраняется как исторический прототип и источник reusable UI, но больше не является обязательным центром разработки.

## Документация

Публичный слой документации ведётся на русском языке как основном источнике.

- [Каноническая концепция](https://github.com/zavx0z/concept) фиксирует математическую модель, миссию, runtime-инварианты, план запуска и разделения репозитория.
- [Философия](docs/PHILOSOPHY.md) объясняет мировоззренческую рамку проекта и роль метафоры как архитектурной дисциплины.
- [Онтология](docs/ONTOLOGY.md) фиксирует, что существует в MetaFor в доменах `Dark`, `Boundary`, `Matrix`, `Energy` и `Bulk`.
...
- [Разработка](docs/DEVELOPMENT.md) фиксирует локальный режим запуска и проверки ядра и междоменных Force-каналов.
...
- [`AGENTS.md`](AGENTS.md) - канонические правила текущей реализации и порядок ближайших изменений,
- [Разработку](docs/DEVELOPMENT.md) - команды запуска, логи и обязательные проверки,
- [Вклад](docs/CONTRIBUTING.md) - правила участия и дисциплина репозитория.
...
MetaFor распространяется под лицензией [GNU Affero General Public License v3.0 or later](docs/LICENSE).
