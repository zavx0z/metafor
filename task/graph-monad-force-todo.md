...
### AUD-001 — Исправить полный набор чтения и записи Reaction

* Status: `READY`
* Dependencies: none
* Scope:
    * `update({a, b})` объявляет оба поля записи;
    * destructuring `const {a, b} = value` объявляет оба поля чтения;
    * Energy не отбрасывает объявленное второе поле;
    * Boundary по-прежнему запрещает запись вне полного разрешённого набора;
    * добавить прямую проверку нескольких полей от DSL до commit.

### AUD-002 — Согласовать проверки Visual с действующим законом

* Status: `READY`
* Dependencies: none
* Scope:
    * убрать старые ожидаемые количества линий `process-read/process-write` из
      четырёх проверок, не возвращая удалённую геометрию к центру Process;
    * заменить проверку прежней плотной решётки Fields на закон центра и одной
      плоской окружности;
    * учитывать допустимую погрешность вычислений с плавающей точкой;
    * `bun run check` проходит полностью, а новые смысловые проверки удаления
      линий Process и кольцевой раскладки остаются зелёными.
