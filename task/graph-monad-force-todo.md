...
* Scope:
    ...
    * возвращать полный набор файлов до записи целевого каталога;
    * не дублировать поведение CLI и не создавать второй генератор Monad.
...
* Dependencies: `MF-206`
* Owner decision:
    * нужны ли отдельные управляющие сообщения доставки, `ACK/NACK`,
      возобновление и канонические курсоры получателей;
    * не считать эту задачу отсутствием действующей устойчивой истории v1 или
      уже реализованного checkpoint replay.
...
* Начальный Git и первый commit от Create MetaFor не доказывают эту модель.
...
* Status: `GATE`
* Dependencies: `MF-302`
* Owner decision: capability и policy для внутреннего агента не следуют
  автоматически из внешнего цикла Codex.
...
* Scope:
    ...
    * явные ограничения ресурсов и проверка Dark Monad;
    * наблюдаемый исход без изменения действующей топологии Лады по умолчанию.
