...
адресом в протоколе.
Порядок полей также не является логической сущностью. Поля адресуются по
`fieldId`, а отображение полевых частиц может быть произвольным или
детерминированным по ID. Логический порядок допускается только там, где он
участвует в переходах superposition/triggers или в значении списка.
...
Порядок полей — не адрес и не логическая сущность.
```
