...
const addAtomVisualFixture = (
	...
	const markerSpecs = Array.from({length: 96}, (_, index) => ({
		...
		kind: index % 2 === 0 ? "field" : "state",
		radius: 5,
		...(() => {
			if (index % 2 === 0) {
				const nucleusIndex = index / 2
				return {
					localX: (nucleusIndex % 4 - 1.5) * 3,
					localY: (Math.floor(nucleusIndex / 4) % 4 - 1.5) * 3,
					localZ: (Math.floor(nucleusIndex / 16) - 1) * 3,
				}
			}
			const stateIndex = (index - 1) / 2
			const majorAngle = Math.PI * 2 * stateIndex / 48
			const tubeAngle = Math.PI * 2 * (stateIndex % 8) / 8
			const ringRadius = 44 + Math.cos(tubeAngle) * 8
			return {
				localX: Math.cos(majorAngle) * ringRadius,
				localY: Math.sin(majorAngle) * ringRadius,
				localZ: Math.sin(tubeAngle) * 8,
			}
		})(),
	}))
	for (const marker of markerSpecs) {
		const position = resolveAtomMarkerPosition(marker)
	...
	const stateMarker = markerSpecs.find((marker) => marker.identity === `state:${atomId}:1`)!
	const statePosition = resolveAtomMarkerPosition(stateMarker)
	...
	const fieldMarker = markerSpecs.find((marker) => marker.identity === `field:${atomId}:0`)!
	const fieldPosition = resolveAtomMarkerPosition(fieldMarker)
