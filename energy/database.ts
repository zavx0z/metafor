...
import type {
  EnergyDatabaseBraneRecord,
  EnergyDatabaseData,
  EnergyDatabaseEntanglementBlockMemberRecord,
  EnergyDatabaseEntanglementBlockRecord,
  EnergyDatabaseEntanglementFieldMemberRecord,
  EnergyDatabaseEntanglementFieldRecord,
  EnergyDatabaseFieldRecord,
  EnergyDatabaseFieldSchemaRecord,
  EnergyDatabaseFieldValueRecord,
  EnergyRuntimeForceData,
  EnergyDatabaseStateSeedConditionRecord,
  EnergyDatabaseStateSeedStateRecord,
  EnergyDatabaseStateSeedTransitionRecord,
  EnergyDbRuntimeOptions,
} from "./database.t.ts"
...
type EnergyRuntimeOperationalCache = {
...
type EnergyRuntimeEntanglementIndex = {
...
type EnergyRuntimeFragmentScope = {
...
const createEnergyRuntimeOperationalCache = (): EnergyRuntimeOperationalCache => ({
...
const createEnergyRuntimeEntanglementIndex = (data: DbData): EnergyRuntimeEntanglementIndex => {
...
const collectEnergyRuntimeFragmentScope = (
  data: DbData,
  wimpId: string,
): EnergyRuntimeFragmentScope => {
  const wimp = data.wimps.find((row) => row.id === wimpId)
  if (!wimp) {
    throw new Error(`Energy runtime fragment missing wimp ${wimpId}`)
  }
...
const collectEnergyRuntimeEntanglementRows = (
  data: DbData,
  scope: EnergyRuntimeFragmentScope,
  entanglementIndex: EnergyRuntimeEntanglementIndex,
...
const collectEnergyRuntimeMetaRows = (
  data: DbData,
  scope: EnergyRuntimeFragmentScope,
): Pick<
  DbData,
...
const collectEnergyRuntimeWimpRows = (
  data: DbData,
  scope: EnergyRuntimeFragmentScope,
...
const cloneFieldSchema = (
  schema: EnergyDatabaseFieldSchemaRecord | DbFieldSchemaRecord,
): EnergyDatabaseFieldSchemaRecord => ({
  type: schema.type,
...
const createLocalRuntimeFieldSignature = (field: EnergyDatabaseFieldRecord): string =>
  JSON.stringify({
    scope: "local",
    key: field.key,
...
const createSeedRuntimeFieldSignature = (semanticKey: string, field: EnergyDatabaseFieldRecord): string =>
  JSON.stringify({
    scope: "seed",
    semanticKey,
    type: field.schema.type,
...
const mapEnergyDatabaseFieldToRuntimeField = (field: EnergyDatabaseFieldRecord): Field => {
  ...
  if (type === "enum" || type.startsWith("enum<")) {
    if (!values) {
      throw new Error(`Energy runtime field '${field.key}' is missing enum values`)
    }
  ...
  throw new Error(`Unsupported DB field type for Energy runtime: ${type}`)
}

const requireEnergyDatabaseFieldValue = (
  data: EnergyDatabaseData,
  fieldIndex: number,
): EnergyDatabaseFieldValueRecord => {
  const fieldValue = data.fieldValues[fieldIndex]
  if (!fieldValue) {
    throw new Error(`Energy database field value missing for field index: ${fieldIndex}`)
  }
  return fieldValue
}

const groupEntanglementBlockMembers = (data: EnergyDatabaseData) => {
...
const groupEntanglementFields = (data: EnergyDatabaseData) => {
...
const groupEntanglementFieldMembers = (data: EnergyDatabaseData) => {
...
const groupStateSeedStates = (data: EnergyDatabaseData) => {
...
const groupStateSeedTransitions = (data: EnergyDatabaseData) => {
...
const groupStateSeedConditions = (data: EnergyDatabaseData) => {
...
const buildEnergyRuntimeFieldRegistry = (data: EnergyDatabaseData) => {
  ...
  const ensureRuntimeField = (signature: string, field: EnergyDatabaseFieldRecord): number => {
    const existing = runtimeFieldIndexBySignature.get(signature)
    if (existing !== undefined) return existing

    const runtimeFieldIndex = runtimeFields.length
    runtimeFields.push(cloneRuntimeField(mapEnergyDatabaseFieldToRuntimeField(field)))
  ...
  for (const seedField of [...data.entanglementFields].sort((left, right) => left.index - right.index)) {
    ...
    if (!representativeField) {
      throw new Error(`Energy runtime seed representative field missing: ${seedField.representativeFieldIndex}`)
    }
    ...
    for (const member of entanglementFieldMembers.get(seedField.index) ?? []) {
      ...
      if (existing !== undefined && existing !== runtimeFieldIndex) {
        throw new Error(`Energy runtime field index mismatch for DB field ${member.fieldIndex}`)
      }
      ...
      if (!wimpFieldId) {
        throw new Error(`Energy runtime field missing Wimp field id for DB field ${member.fieldIndex}`)
      }
...
const prepareEnergyEntanglementProjection = (
  data: EnergyDatabaseData,
    ...
    blocks: [...data.entanglementBlocks]
      ...
      .map((block) => ({
        ...
        fields: (blockFields.get(block.index) ?? []).map((seedField) => {
          const representativeField = data.fields[seedField.representativeFieldIndex]
          if (!representativeField) {
            throw new Error(`Energy runtime entanglement representative field missing: ${seedField.representativeFieldIndex}`)
          }

          const runtimeFieldIndex = runtimeFieldIndexByDbFieldIndex[representativeField.index]
          if (runtimeFieldIndex === undefined) {
            throw new Error(`Energy runtime field index missing for DB field ${representativeField.index}`)
          }
          ...
          for (const member of fieldMembers.get(seedField.index) ?? []) {
            if (runtimeFieldIndexByDbFieldIndex[member.fieldIndex] !== runtimeFieldIndex) {
              throw new Error(`Energy runtime entanglement member ${member.fieldIndex} resolves to different runtime field`)
            }
          }
...
const prepareEnergyStateSeedGraph = (
  braneIndex: number,
  runtimeFieldIndexByDbFieldIndex: number[],
  ...
  if (states.length === 0) {
    throw new Error(`Energy state seeds missing for brane ${braneIndex}`)
  }

  const initialState = states.find((state) => state.initial)
  if (!initialState) {
    throw new Error(`Energy initial state seed missing for brane ${braneIndex}`)
  }
    ...
    collapses: states.map((state) => {
      ...
      return transitions.map((transition) => {
        ...
        rawConditions.forEach((condition) => {
          ...
          if (runtimeFieldIndex === undefined) {
            throw new Error(`Energy runtime field missing for DB field ${condition.fieldIndex}`)
          }

          if (Object.prototype.hasOwnProperty.call(conditions, runtimeFieldIndex)) {
            throw new Error(
              `Energy state seed transition ${transition.index} has duplicate runtime field ${runtimeFieldIndex}`,
            )
          }
...
const prepareEnergyDatabaseData = (rawData: DbData): EnergyDatabaseData => {
  ...
  const branes: EnergyDatabaseBraneRecord[] = []
  const fields: EnergyDatabaseFieldRecord[] = []
  const fieldValues: EnergyDatabaseFieldValueRecord[] = []

  orderedWimps.forEach((wimp, braneIndex) => {
    const meta = metaById.get(wimp.metaId)
    if (!meta) {
      throw new Error(`Energy database missing meta ${wimp.metaId} for wimp ${wimp.id}`)
    }
    ...
    wimpFields.forEach((wimpField) => {
      ...
      if (!metaField) {
        throw new Error(`Energy database missing meta field ${wimpField.metaFieldId} for wimp field ${wimpField.id}`)
      }
      ...
      if (!value) {
        throw new Error(`Energy database missing field value for wimp field ${wimpField.id}`)
      }
  ...
  const entanglementBlocks: EnergyDatabaseEntanglementBlockRecord[] = []
  const entanglementBlockMembers: EnergyDatabaseEntanglementBlockMemberRecord[] = []
  const entanglementFields: EnergyDatabaseEntanglementFieldRecord[] = []
  const entanglementFieldMembers: EnergyDatabaseEntanglementFieldMemberRecord[] = []
    ...
    .forEach(([membershipKey, families], blockIndex) => {
      ...
      blockMembers.forEach((member) => {
        ...
        if (braneIndex === undefined) {
          throw new Error(`Energy database cannot resolve entanglement member wimp ${member.wimpId}`)
        }
      ...
      orderedFamilies.forEach(({ seedField, fieldMembers }, blockFieldIndex) => {
        ...
        if (representativeFieldIndex === undefined) {
          throw new Error(`Energy database cannot resolve entanglement representative field ${seedField.representativeWimpFieldId}`)
        }

        const representativeBraneIndex = fields[representativeFieldIndex]?.ownerBraneIndex
        if (representativeBraneIndex === undefined) {
          throw new Error(`Energy database cannot resolve representative brane for field ${seedField.representativeWimpFieldId}`)
        }
        ...
        fieldMembers.forEach((member) => {
          ...
          if (fieldIndex === undefined || braneIndex === undefined) {
            throw new Error(`Energy database cannot resolve entanglement field member ${member.id}`)
          }
  ...
  const stateSeedStates: EnergyDatabaseStateSeedStateRecord[] = []
  const stateSeedTransitions: EnergyDatabaseStateSeedTransitionRecord[] = []
  const stateSeedConditions: EnergyDatabaseStateSeedConditionRecord[] = []

  orderedWimps.forEach((wimp) => {
    const braneIndex = braneIndexByWimpId.get(wimp.id)
    if (braneIndex === undefined) {
      throw new Error(`Energy database missing brane index for wimp ${wimp.id}`)
    }
      ...
      transitions.forEach((transition) => {
        ...
        ;(metaTransitionConditionsByTransitionId.get(transition.id) ?? []).forEach((condition) => {
          ...
          if (fieldIndex === undefined) {
            throw new Error(`Energy database cannot resolve state condition field for meta field ${condition.metaFieldId}`)
          }
...
export const prepareEnergyRuntimeForceData = (
  rawData: DbData,
): EnergyRuntimeForceData => {
  const normalizedData = normalizeDbData(rawData)
  const data = prepareEnergyDatabaseData(normalizedData)
  const { runtimeFieldIndexByDbFieldIndex, runtimeFieldIndexToWimpFieldIds } = buildEnergyRuntimeFieldRegistry(data)
  ...
  for (const field of data.fields) {
    ...
    if (runtimeFieldIndex === undefined) {
      throw new Error(`Energy runtime force data missing runtime field index for DB field ${field.index}`)
    }
...
const mergeEnergyRuntimeFragments = (fragments: Iterable<DbData>): DbData => {
...
const ensureEnergyRuntimeMetaRows = async (
  backend: DbBackend,
  cache: EnergyRuntimeOperationalCache,
  metaId: string,
): Promise<DbMetaRows> => {
  ...
  if (!rows) {
    throw new Error(`Energy runtime fragment missing meta ${metaId}`)
  }
...
const ensureEnergyRuntimeWimpRows = async (
  backend: DbBackend,
  cache: EnergyRuntimeOperationalCache,
  wimpId: string,
): Promise<DbWimpRows> => {
...
const ensureEnergyRuntimeWimpField = async (
  backend: DbBackend,
  cache: EnergyRuntimeOperationalCache,
  wimpFieldId: string,
  ...
  if (!field) {
    throw new Error(`Energy runtime fragment missing wimp field ${wimpFieldId}`)
  }
...
const ensureEnergyRuntimeFieldSource = async (
  backend: DbBackend,
  cache: EnergyRuntimeOperationalCache,
...
const ensureEnergyRuntimeWimpEdge = async (
  backend: DbBackend,
  cache: EnergyRuntimeOperationalCache,
  childWimpId: string,
...
const ensureEnergyRuntimeEntanglementFamily = async (
  backend: DbBackend,
  cache: EnergyRuntimeOperationalCache,
...
const resolveEnergyRuntimeRootFieldId = async (
  backend: DbBackend,
  cache: EnergyRuntimeOperationalCache,
  wimpFieldId: string,
): Promise<string> => {
  ...
  for (;;) {
    if (visited.has(currentFieldId)) {
      throw new Error(`Energy runtime fragment detected cyclic field_sources at ${currentFieldId}`)
    }
    visited.add(currentFieldId)

    const source = await ensureEnergyRuntimeFieldSource(backend, cache, currentFieldId)
...
const collectOperationalPackageWimpIds = async (
  ...
  while (queue.length > 0) {
    ...
    const rows = await ensureEnergyRuntimeWimpRows(backend, cache, nextWimpId)
    await ensureEnergyRuntimeMetaRows(backend, cache, rows.wimp.metaId)
    await ensureEnergyRuntimeWimpEdge(backend, cache, nextWimpId)

    for (const source of rows.sources) {
      const parentField = await ensureEnergyRuntimeWimpField(backend, cache, source.parentWimpFieldId)
...
const appendOperationalPackageRows = (
  ...
  for (const packageWimpId of packageWimpIds) {
    const rows = requireCached(cache.wimpRowsById.get(packageWimpId), `Energy runtime fragment cache missing wimp ${packageWimpId}`)
    const metaRows = requireCached(
      cache.metaRowsById.get(rows.wimp.metaId),
      `Energy runtime fragment cache missing meta ${rows.wimp.metaId}`,
    )
...
const collectOperationalEntanglementIds = async (
  ...
  for (const field of fragment.wimpFields) {
    const rootFieldId = await resolveEnergyRuntimeRootFieldId(backend, cache, field.id)
...
const appendOperationalEntanglements = async (
  ...
  for (const entanglementId of entanglementIds) {
    const family = await ensureEnergyRuntimeEntanglementFamily(backend, cache, entanglementId)
    if (!family) continue
...
const prepareEnergyRuntimeFragmentFromOperationalCache = async (
  backend: DbBackend,
  cache: EnergyRuntimeOperationalCache,
  wimpId: string,
): Promise<DbData> => {
...
export const prepareEnergyRuntimeLoadedFragmentFromDbOperational = async (
  backend: DbBackend,
  ...
  const cache = createEnergyRuntimeOperationalCache()
  const fragments = await Promise.all(
    requestedWimpIds.map((wimpId) => prepareEnergyRuntimeFragmentFromOperationalCache(backend, cache, wimpId)),
  )
  return mergeEnergyRuntimeFragments(fragments)
}

const prepareEnergyRuntimeFragment = (
  rawData: DbData,
  wimpId: string,
): DbData => {
  const data = normalizeDbData(rawData)
  const entanglementIndex = createEnergyRuntimeEntanglementIndex(data)
  const scope = collectEnergyRuntimeFragmentScope(data, wimpId)
  const metaRows = collectEnergyRuntimeMetaRows(data, scope)
  const wimpRows = collectEnergyRuntimeWimpRows(data, scope)
  const entanglementRows = collectEnergyRuntimeEntanglementRows(data, scope, entanglementIndex)
...
const prepareEnergyRuntimeLoadedFragment = (
  rawData: DbData,
  activeWimpIds?: Iterable<string>,
): DbData => {
  const data = normalizeDbData(rawData)
  ...
  return mergeEnergyRuntimeFragments(requestedWimpIds.map((wimpId) => prepareEnergyRuntimeFragment(data, wimpId)))
}

const prepareEnergyWriteData = (
  data: EnergyDatabaseData,
  options: EnergyDbRuntimeOptions = {},
): Data => {
  const { runtimeFields, runtimeFieldIndexByDbFieldIndex } = buildEnergyRuntimeFieldRegistry(data)
  ...
  const entanglement = options.entanglement ?? prepareEnergyEntanglementProjection(data, runtimeFieldIndexByDbFieldIndex)
  ...
  return {
    ...
    branes: data.branes.map((brane) => {
      const stateGraph = prepareEnergyStateSeedGraph(
        brane.index,
      ...
      return {
        values: data.fields
          ...
          .map((field) => {
            const runtimeFieldIndex = runtimeFieldIndexByDbFieldIndex[field.index]
            if (runtimeFieldIndex === undefined) {
              throw new Error(`Energy runtime field index missing for DB field ${field.index}`)
            }

            return [runtimeFieldIndex, structuredClone(requireEnergyDatabaseFieldValue(data, field.index).value)] as [
              number,
              BraneValue,
            ]
...
const prepareEnergyStoreFromDatabase = (
  data: EnergyDatabaseData,
  options: EnergyDbRuntimeOptions = {},
): PreparedData => assembleStoredEnergyData(flattenEnergyData(prepareEnergyWriteData(data, options)))

export const prepareEnergyRuntimeData = (
  rawData: DbData,
  options: EnergyDbRuntimeOptions = {},
): Data => prepareEnergyWriteData(prepareEnergyDatabaseData(rawData), options)

export const prepareEnergyRuntimeStore = (
  rawData: DbData,
  options: EnergyDbRuntimeOptions = {},
): PreparedData => prepareEnergyStoreFromDatabase(prepareEnergyDatabaseData(rawData), options)

export const prepareEnergyRuntimeStoreFromDb = (
  backend: DbBackend,
  options: EnergyDbRuntimeOptions = {},
): Promise<PreparedData> =>
  prepareEnergyRuntimeLoadedFragmentFromDbOperational(backend).then((fragment) =>
    prepareEnergyRuntimeStore(fragment, options),
  )
