...
import { openDbMaterializationWriter } from "@metafor/boundary/materialize"
import { openDbSqliteBackend } from "@metafor/boundary/db/sqlite"
import { prepareRuntimeData, prepareRuntimeStore } from "../energy.ts"
...
describe("energy runtime projection from db data", () => {
  test("проецирует canonical db data напрямую в energy runtime input", async () => {
  ...
  test("собирает prepared energy store из canonical db data", async () => {
