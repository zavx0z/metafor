...
import { flattenEnergyData, prepareData } from "../energy"
import type { EnergyData, EnergyTransitionRecord } from "../store.t"
import { FieldType, type Data } from "../gravity"
import { OP } from "../weak/constants"

function getBraneLocalValues(store: EnergyData, braneIndex: number) {
...
function getBraneSharedBlockIds(store: EnergyData, braneIndex: number) {
  const brane = store.branes[braneIndex]
  if (!brane) {
    return []
  }
...
function getSharedBlockValues(store: EnergyData, blockIndex: number) {
...
function getBraneStateTransitions(store: EnergyData, braneIndex: number, stateIndex: number) {
  const brane = store.branes[braneIndex]
  ...
  return store.transitions
    ...
    .map((transition: EnergyTransitionRecord) => ({
...
describe("prepareData — подготовка canonical JS store", () => {
  test("Energy flattening должен переводить nested conditions в parsed checks", () => {
    const data: Data = {
    ...
    const flattened = flattenEnergyData(data)
