...
import type { EnergyData } from "../store.t"
...
describe("prepared entanglement projection", () => {
  test("energy не выводит entanglement из одинаковых values без projection", () => {
    const data: Data = {
  ...
  test("energy materializes shared блоки только из prepared projection", () => {
    const data: Data = {
      fields: [
        { type: FieldType.F32 },
        { type: FieldType.F32 },
  ...
  test("energy валидирует projection и не materializes расходящиеся shared values", () => {
    const data: Data = {
      fields: [{ type: FieldType.F32 }],
      branes: [
        { values: [[0, 100]], state: 0, collapses: [[null]] },
