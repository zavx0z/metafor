...
describe("CPU runtime — scenario tests", () => {
  test("simpleTransition — 1 brane hp > 50", async () => {
    const runtime = new CPUWeakRuntime(createIsolatedStore(createSimpleBraneFixture()))
  ...
  test("multipleBranes — 3 branes different conditions", async () => {
    const runtime = new CPUWeakRuntime(createIsolatedStore(createMultipleBranesFixture()))
  ...
  test("lockFlag — locked brane does not transition", async () => {
    const runtime = new CPUWeakRuntime(createIsolatedStore(createLockedBraneFixture()))
