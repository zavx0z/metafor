/**
 * `@energy/weak/gpu/pack` кодирует значения из канонического store в производные execution-формы GPU.
 *
 ...
 */
export function fieldTypeToBytecodeType(fieldType: EnergyFieldRecord["type"]): number {
  switch (fieldType) {
    case FIELD_TYPE.F32:
 ...
 */
export function encodeValue(value: EnergyValue, ctx: PackContext): EncodedValue {
  if (ctx.enum) {
  ...
  if (ctx.type === VALUE_TYPE.ARRAY) {
    ...
    if (ctx.allocateHeap && ctx.heap) {
      ...
      for (let index = 0; index < arr.length; index++) {
        ...
        ctx.heap[ptr + 1 + index] = encodeValue(arr[index] as EnergyValue, itemContext).value1
      }
