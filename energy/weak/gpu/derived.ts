/**
 * `@energy/weak/gpu/derived` выводит производные execution-формы из канонического Energy store.
 *
...
function createFieldMetaMap(fields: EnergyFieldRecord[]): Map<number, { fieldType: number; fieldSize: number }> {
...
function groupTransitionConditions(
  ...
  const grouped = new Map<number, EnergyConditionRecord[]>()
...
function toFlattenedTransitions(store: EnergyData): Array<{ transitions: FlattenedTransition[][] }> {
...
function collectSharedBlockFields(store: EnergyData): EnergyFieldValueRecord[][] {
  return store.sharedBlocks.map((block) => {
    const fields: EnergyFieldValueRecord[] = []
...
function collectBraneLocalFields(store: EnergyData): EnergyFieldValueRecord[][] {
  return store.branes.map((brane) => {
    const fields: EnergyFieldValueRecord[] = []
...
function collectBraneSharedBlockRefs(store: EnergyData): number[][] {
  return store.branes.map((brane) => {
    const refs: number[] = []
...
function countArrayWords(fields: EnergyFieldRecord[], values: EnergyFieldValueRecord[]): number {
  let words = 0
...
function createLocalFields(
  braneFields: EnergyFieldValueRecord[][],
  fields: EnergyFieldRecord[],
  stringTable: string[],
  fieldMetaMap: Map<number, { fieldType: number; fieldSize: number }>,
  allocateHeap?: (size: number) => number,
...
/**
 * Выводит packed-данные Weak из канонического Energy store.
 *
 ...
 */
export function deriveWeakData(store: EnergyData): DerivedWeakData {
