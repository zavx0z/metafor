 ...
 *
 * Каноническая истина остаётся в Energy store, а этот runtime держит только
...
export class GPUWeakRuntime implements WeakRuntime {
  ...
  private readonly store$: EnergyStore
  private lastStates: number[]
  private pending: Promise<void> = Promise.resolve()

  private constructor(context: GpuRuntimeContext, store$: EnergyStore) {
    this.context = context
    this.store$ = store$
    this.lastStates = [...store$.states]
  }

  static async create(device: GPUDevice, store$: EnergyStore): Promise<GPUWeakRuntime> {
  ...
  private resolveArrayWrites(
    ...
    valueOffset: number,
    fieldIndex: number,
    value: EnergyStore["braneValues"][number]["value"],
  ): { writes: Array<{ offset: number; value1: number; value2?: number }>; heapMirror?: Uint32Array } | null {
