...
- Boundary доставляет atom и каждый process descriptor отдельным Graviton;
...
Каждое atom/process изменение приходит отдельным `ForceMessage` с одной
`Particle`; `replace` содержит только delta адресованной entity. Energy не
принимает aggregate catalog и не использует reset.

Replay после cold start/reconnect состоит из обычных atom/process `add`.
...
scope на этом этапе — atom+wimp: `${wimp}\0${atomId}`; lifetime равен lifetime
...
field key, не by fieldId. `self` содержит `{atom, meta, path}` для atom.
