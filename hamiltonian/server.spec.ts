...
import {
  createHamiltonianTestServer,
} from "./server/test-harness.ts"
import {hamiltonianServerBootstrapDeclaration} from "./server/lifecycle.ts"
import {WeriftPeer, type PeerSignal} from "./server/peer/werift-peer.ts"

const running: Array<ReturnType<typeof createHamiltonianTestServer>> = []
...
async function registerTestPushSubscription(
  host: ReturnType<typeof createHamiltonianTestServer>,
  workerIdentity: string,
  deviceId: string,
  endpoint: string,
  frames?: Array<Record<string, unknown>>,
...
describe("isolated Hamiltonian host", () => {
  ...
  test("keeps page source revision stable across cold hosts with identical browser bundles", async () => {
    ...
    const hostA = createHamiltonianTestServer({
      port: 0,
      token: "source-host-a",
      identity: "stable-source-host",
      browserBundles,
    })
    const hostB = createHamiltonianTestServer({
      port: 0,
      token: "source-host-b",
      identity: "stable-source-host",
      browserBundles,
  ...
  test("does not publish browser source or release materialization after a rejected artifact", async () => {
    ...
    const host = createHamiltonianTestServer({
      port: 0,
  ...
  test("rejects a Service Worker identity whose code version is not SemVer", async () => {
    const host = createHamiltonianTestServer({port: 0, token: "test-token"})
    running.push(host)
    const controlUrl = new URL("/control", host.server.url)
    controlUrl.protocol = "ws:"
    controlUrl.searchParams.set("token", host.token)
  ...
  test("supplies fresh host capability through encrypted Push after Bun restart", async () => {
    ...
    const first = createHamiltonianTestServer({
      port: 0,
    ...
    const restarted = createHamiltonianTestServer({
      port: 0,
  ...
  test("sends retained current state and a causal frontier before live control messages", async () => {
    const host = createHamiltonianTestServer({port: 0, token: "test-token", heartbeatMs: 10_000})
    running.push(host)
    await host.bunReady
  ...
  test("replaces host A with host B in one retained page projection without reloading the browser profile", async () => {
    ...
    const hostA = createHamiltonianTestServer({port: 0, token: "host-a-token", identity, heartbeatMs: 10_000})
    running.push(hostA)
    ...
    await Bun.sleep(2)
    const hostB = createHamiltonianTestServer({port: 0, token: "host-b-token", identity, heartbeatMs: 10_000})
    running.push(hostB)
  ...
  test("expires an abruptly closed browser profile only after its reconnect grace", async () => {
    ...
    const host = createHamiltonianTestServer({port: 0, token: "test-token", heartbeatMs})
    running.push(host)
    await host.bunReady
  ...
  test("cold-rebirths the Bun embodiment as a new OS process without another listener", async () => {
    const host = createHamiltonianTestServer({port: 0, token: "test-token", version: "v-process"})
    running.push(host)
  ...
  test("serializes concurrent rebirths of one Bun role", async () => {
    const host = createHamiltonianTestServer({port: 0, token: "test-token", version: "v-race"})
    running.push(host)
  ...
  test("does not birth a Bun process after host shutdown has started", async () => {
    const host = createHamiltonianTestServer({port: 0, token: "test-token", placement: "server"})
    running.push(host)
    await host.bunReady
  ...
  test("elects one Window across two device sockets and replaces a lost leader", async () => {
    const host = createHamiltonianTestServer({port: 0, token: "test-token", heartbeatMs: 1_000})
    running.push(host)
    const controlUrl = new URL("/control", host.server.url)
    controlUrl.protocol = "ws:"
  ...
  test("distinguishes one acknowledged connection from a reconnect epoch", async () => {
    const host = createHamiltonianTestServer({port: 0, token: "test-token", heartbeatMs: 100})
    running.push(host)
    const controlUrl = new URL("/control", host.server.url)
    controlUrl.protocol = "ws:"
    controlUrl.searchParams.set("token", "test-token")
