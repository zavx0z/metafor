...
test("builds the real orchestration and isolated layout Worker bundles", async () => {
  ...
  try {
    ...
    const publicationTypeScript = await Bun.file(join(
      repositoryRoot,
      "hamiltonian/server/browser/publication.ts",
    )).text()
    expect(publicationTypeScript).toContain('const updateRoot = `${experimentRoot}/update`')
    const watchRootsStart = publicationTypeScript.indexOf("for (const root of [")
    const watchRootsEnd = publicationTypeScript.indexOf("]) {", watchRootsStart)
    expect(publicationTypeScript.slice(watchRootsStart, watchRootsEnd)).toContain("updateRoot")
