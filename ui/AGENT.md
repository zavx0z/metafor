...
* HUD/window panes считаются как единая flex-раскладка: title/header slot сверху и content slot внутри окна. Если повторяется chrome/layout окна, расширяй или используй `ui/panes/pane-frame.ts`, а не заводи локальную параллельную геометрию рядом с конкретной pane.
...
* Header/title стандартных panes должен идти через общий title bar path (`HudWindowTitleBar` для HUD-style panes), чтобы слева были minimize/actions, центрированный title/subtitle и правые actions. Локальный header допустим только для нестандартной surface с явной причиной.
