...
* После изменений в `@metafor/elements`, которые используются в `@metafor/components`, нужно перезапускать `components playground` перед визуальной проверкой. Нельзя считать результат в браузере достоверным, пока не подтверждено, что runtime подхватил изменения зависимого пакета.
