...
- Отдельную пользовательскую задачу или чат Codex для существующей проектной
  ...
  предъявляет владельцу как предложение новой задачи. Техническое имя
  субагента выводить из номера подзадачи: lowercase, `-` и `.` заменить на `_`
  (`MF-428.5` → `mf_428_5`); во всех сообщениях сохранять полный номер, название
  и ссылку на запись подзадачи в карточке родителя.
