...
- ✅ Обработка интерполяций `${...}` (унифицированный формат template/items для смешанного контента)
...
### Интерполяции внутри массивов (адресация путями и template/items)
...
- Атрибуты внутри элементов массивов:
  ...
  - Смешанный контент: `class="g-${group.id} i-${item.id}"` →
    `{ template: "g-${0} i-${1}", items: [ { src: ["core", "groups"], key: "id" }, { src: ["core", "groups", "items"], key: "id" } ] }`
  - Несколько `${...}` в значении нормализуются в template с индексами `${0}`, `${1}`, ...
...
- Условные атрибуты внутри массивов:
  ...
  - В смешанных значениях условные подстановки обрабатываются до унификации и затем входят в items шаблона
...
5. Парсит HTML структуру и восстанавливает источники данных
   ...
   - Для атрибутов: унифицированный формат `template/items`; для простой интерполяции допустим `{ src, key }`
