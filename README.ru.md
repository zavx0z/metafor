...
MetaFor построен на двух основных сущностях:

### Мета

**Мета** — это схема, описывающая структуру и поведение актора.

Мета определяет:

- **Контекст** — структуру данных актора
- **Состояния** — возможные состояния и переходы между ними
- **Процессы** — асинхронные действия и их обработчики
- **Реакции** — реакции на события от других акторов
- **Представление** — визуальное отображение (render и style)

Мета создаётся декларативно через API `MetaFor()` из пакета `@metafor/meta`. Это статическое описание, которое определяет, как будет вести себя актор, но не является самим актором.

**Пакет**: `@metafor/meta`

Пакет построен для разработчиков — это удобный конструктор с полной поддержкой типов и автодополнением в VanillaJS, что позволяет разработчику без компиляции объявлять и редактировать декларативную схему атома.

Схема, созданная через `MetaFor()`, является конечными данными пакета. ИИ-агенты и NoCode конструкторы используют саму схему напрямую, а разработчик работает с конструктором MetaFor для плавного перехода от кода к визуализированному представлению архитектуры и кода с ИИ-агентами.

### Атом

**Атом** — это активный экземпляр, созданный из мета.

Атом:

- Существует в вычислительном пространстве как активный участник
- Имеет собственное состояние (контекст, текущее состояние)
- Выполняет процессы и реагирует на события
- Взаимодействует с другими атомами через события и реакции
- Управляется согласно [Quantum Theory of Programming (qTp)](atom/README.ru.md)

Атом создаётся из мета и становится живой сущностью в системе. Одна мета может породить множество атомов, каждый со своим состоянием и жизненным циклом.
...
Инструменты отладки атомов с пошаговым выполнением, снимками состояния и воспроизведением.

- `./web/logger` — логирование в браузере
- `./web/debugger` — отладчик с остановкой времени
- `./server` — серверное логирование
...
- [x] [`@metafor/virtual`](infra/virtual/README.ru.md) - визуализация виртуальными частицами в процессе инициализации зависимостей графической системы
