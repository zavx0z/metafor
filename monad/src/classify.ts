...
// Глобальные массивы данных (Global Context Heaps)
@group(0) @binding(0)
...
// Текущие состояния монад (Read-Only в этом проходе)
@group(0) @binding(2)
var<storage, read> states: array<u32>;

// Новые состояния монад (Write-Only)
@group(0) @binding(3)
var<storage, read_write> newStates: array<u32>;

// Карта контекста: [monadId * stride + localIdx] -> globalIdx
// Связывает локальные поля монады с глобальной кучей.
@group(0) @binding(4)
var<storage, read> contextMap: array<u32>;

// Байт-код логики переходов
@group(0) @binding(5)
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // 1. Получение дескриптора состояния
    // bytecode[tableOffset + stateID] -> указатель на Блок Состояния
    let stateBlockPtr = bytecode[params.tableOffset + currentState];
    let transitionCount = bytecode[stateBlockPtr];

    // 2. Итерация по переходам
    var i = 0u;
    loop {
        ...
        // Блок перехода: [targetState, conditionsPtr]
        ...
        loop {
            ...
            // Блок проверки: [type, localIdx, op, refVal]
            ...
            // Резолв глобального индекса через карту контекста
            ...
            if (dtype == TYPE_FLOAT) {
                ...
                // UINT или BOOL (хранятся как uint)
