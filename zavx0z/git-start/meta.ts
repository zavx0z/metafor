...
export default MetaFor("git-start")
  ...
  .view({
    render: ({ context, html }) => html`
      ${context.operation === "clone" && html`<meta-for src="zavx0z/git-start-clone" context=${{ command: context.command, args: context.args }} />`}
      ${context.operation === "init" && html`<meta-for src="zavx0z/git-start-init" context=${{ command: context.command, args: context.args }} />`}
