...
## Owner rejection для NODES-008.5

Portrait crop `13.42.51` от 12 августа 2026 года показывает две одинаковые
правые EAST-дорожки: между правой границей page compound и ближайшей занятой
vertical lane остаётся лишний пустой pitch. Оригинал передан как temporary
screencapture attachment и пока не копировался в Git.

Это новый зеркальный дефект после сохранённого checkpoint NODES-008.4
`b0fee1ee0`; он не отменяет machine proof левой стороны. До изменения solver
machine-readable live witness из первой открытой вкладки подтвердил два
одинаковых нарушения при `clearance=28`: `524.7 → 580.7` и
