...
const gitValue = (args: readonly string[]): string => {
  const process = Bun.spawnSync(["git", ...args], {cwd: repository})
  if (process.exitCode !== 0) throw new Error(process.stderr.toString())
  return process.stdout.toString().trim()
}

const report = {
  kind: "NODES-001-layout-benchmark",
  measuredAt: new Date().toISOString(),
  gitHead: gitValue(["rev-parse", "HEAD"]),
  layoutTree: gitValue(["rev-parse", "HEAD:pkg/nodes/layout"]),
  runtime: {
    bun: Bun.version,
    platform: platform(),
    release: release(),
  ...
  results: [
    benchmark("live-layout-request-landscape.json"),
    benchmark("live-layout-request-portrait.json"),
  ],
}
