...
Действующий
[контракт сред выполнения](../cosmos/README.md#среды-выполнения-packages)
явно разделяет browser и Bun environments, закрепляет смысл каждого env и
запрещает сборке считать неизвестную небраузерную среду Bun-средой по
умолчанию.
...
Действующие законы раскладки находятся в [`../quantum/bulk/VISUAL.md`](../quantum/bulk/VISUAL.md)
