...
Model Service и Chat Send Tool. Ближайшая цель — не самоизменение Лады, а
завершённый предметный цикл одного адресованного сообщения:
...
После полной остановки новый contour обязан заново создать живые Energy
resources и executions. Сохранённые State и Fields не являются доказательством
живой SSO-сессии или WebSocket. Истёкшая SSO-сессия возвращает Auth в
проверяемый телефонный/SMS-путь. Hot reload и частичное оживление доменов для
этого не вводятся.
...
1. завершить полный новый запуск `SYS-001` и рабочий адресованный цикл Лады
   `LAD-001`—`LAD-004`;
