...
Новая реализация создаётся с нуля рядом в пакетах `web`, `server` и
`interface`. Каждый из них разделяется на самостоятельные `import`, `service`
и `update`. Сначала материализуется только чистая пакетная структура без
переноса прежнего кода. Browser loader реализуют `web/import` и `web/service`;
server-аналоги появляются позднее, а общий договор выделяется в `interface`
только после двух фактических реализаций.
...
прототипу и не переносится в новые пакеты. Новые `web/update`, `server/update`
и `interface/update` начинают наполняться только собственными последовательными
срезами после появления соответствующего загрузочного поведения.
