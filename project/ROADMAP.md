...
Ближайшее runtime-упрощение — один слушающий Dark server на Вселенную:
...
через Dark gateway. Этот срез `MF-409` сохраняет payload и routing и оставляет
замену физической пары каналов на WebRTC отдельным будущим transport-этапом.
...
1. свести runtime одной Вселенной к единственному Dark listener в `MF-409` и
   проверить параллельное рождение нескольких contour;
