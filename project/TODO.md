...
Bulk и `pkg/visual` не входят в работу. Текущий диагностический срез
`HAM-002.1` фиксирует полный import graph и атомарную последовательность;
отдельный legacy fallback screen удаляется последующим зарегистрированным
срезом, после чего нодовый canvas остаётся единственным отображением.
