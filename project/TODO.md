...
каждого контура` формулирует общий Hamiltonian lifecycle contract. Она не
запущена автоматически: состояние `READY`, первый срез `HAM-001.1` владелец
...
разрешённый срез `MF-428.4 — Принудительно обновлять Worker всех профилей с
сервера` переведён в `IN_PROGRESS` только для read-only исследования контракта;
product patch и live-управление профилями ещё не начаты.
