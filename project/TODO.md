...
принят. Текущий срез `HAM-001.2 — Показывать действующее WebRTC-соединение между
браузером и сервером` восстанавливает две фактически открытые линии Oracle и
Force между exact RTC endpoints через declaration boundary и не относится к
layout или renderer.

Текущие задачи Codex: NODES-008.5 — не удваивать правый боковой corridor в
`DOWN`; HAM-001.2 — показывать действующее WebRTC-соединение между браузером и
сервером.
