...
Текущая задача Codex: `MF-428.5 — Нормализовать шапку ноды Service Worker`;
срез передаётся отдельной задаче Codex с тем же именем. По решению владельца
MF-428.5 выполняется перед MF-428.4, поскольку её единственная зависимость
MF-428.3 уже принята. MF-428 остаётся `IN_PROGRESS`, а MF-428.4 остаётся
следующим срезом после отдельной приёмки MF-428.5.
