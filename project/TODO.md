...
и причины оставшегося JavaScript. Текущий `HAM-003.2` делает корневой server
entrypoint явным и тонким без изменения host mechanisms; последующие срезы
меняют по одному механизму и сохраняют договоры `HAM-002` и `UPD-002`.
