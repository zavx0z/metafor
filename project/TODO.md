...
flowchart LR
    ...
    NODES007["NODES-007 · одинаковая раскладка после reload"]
...
вместе с владельцем`. Подзадача `MF-424.1` повторно принята 11 августа
2026 года: серверная часть остаётся в едином видимом контуре после
rebirth и диагностических подключений. Текущий выбранный владельцем срез —
`MF-424.3`: все одновременно открытые вкладки должны находиться внутри одного
Chrome и видеть одинаковый retained lifecycle друг друга. Зарегистрированная
`MF-424.2` про устойчивые цвета transport family и легенду в панели `Вид
холста` остаётся следующей в очереди. Параллельно `MF-425 — Управлять одной Вселенной на
...
вариантами` проходит review общего выбора маршрутов в `@nodes/layout`, а
`NODES-007 — Сохранять одинаковую раскладку после перезагрузки` устраняет
зависимость placement от смены непрозрачных runtime IDs.

Текущая задача Codex: `MF-424.3`; `NODES-007` передана на closing review.
...
| NODES-007 | REVIEW      | нет       | [Открыть](tasks/NODES-007.md) |
