...
flowchart LR
    ...
    MF428["MF-428 · Service Worker без открытых вкладок"]
    WEBPUSH001["WEBPUSH-001 · переиспользуемый Web Push"]
...
вместе с владельцем`; её текущая подзадача `MF-424.1` собирает серверную часть
в единый видимый контур. Следом зарегистрирована `MF-424.2`: устойчивые цвета
transport family и легенда в панели `Вид холста`. Параллельно `MF-425 — Управлять одной Вселенной на
...
Текущая задача Codex: `MF-424.1 — повторно закрепить RTCPeerConnection сервера внутри Peer process`.
`WEBPUSH-001 — переиспользуемый Web Push` передан в closing review точного
result commit.
...
| WEBPUSH-001| REVIEW      | нет            | [Открыть](tasks/WEBPUSH-001.md)|
| MF-428     | WAITING     | WEBPUSH-001    | [Открыть](tasks/MF-428.md)     |
