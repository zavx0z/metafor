...
Новая линия `UPD` начинается с `UPD-001 — Собрать механизм обновлений
Hamiltonian в одной директории`. Задача переносит уже работающий локальный
механизм под `hamiltonian/update` без изменения его поведения и без добавления
межмашинной доставки либо обновления серверных процессов. Задача начата;
первый срез `UPD-001.1 — Зафиксировать import graph механизма обновлений`
снимает полный ownership/import inventory без перемещения product source.
