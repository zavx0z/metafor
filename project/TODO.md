...
сигналом будущего update. Текущий `LOAD-001.13 — Не отклонять запрос
отсутствующего asset без сети` добавляет контролируемый response для lazy cache
miss. `UPD-002 — Обновлять всю клиентскую сборку через Service Worker` ждёт
результата `LOAD-001`.
