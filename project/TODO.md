...
flowchart LR
    ...
    UPD001["UPD-001 · единая директория обновлений"]
...
Линия `UPD` началась с `UPD-001 — Собрать механизм обновлений Hamiltonian в
одной директории`. Result commit перенёс действующий локальный механизм под
`hamiltonian/update` без добавления межмашинной доставки или обновления
серверных процессов. Независимый closing review принял product structure и
automated corpus, но вернул UPD-001.11 на повтор того же двухпрофильного live
пути с сохранением sanitized raw evidence; до положительного повторного
