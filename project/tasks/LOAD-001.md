...
* Исторические checkpoints `HAM-003.1`–`HAM-003.10` сохраняют структуру прототипа
  без изменения behavior, `MF-411` — общий закон Hamiltonian, а `MF-425` —
  среду однодевайсной приёмки; они не заменяют предметный loader contract.
