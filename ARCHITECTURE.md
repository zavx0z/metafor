...
Такое чтение не требует отдельного исполняемого модуля или корневой директории `dark/`.
...
4. перестройка скрытой организации через структурированные изменения,
...
Boson — общий тип переносимой единицы.

1. `Gravity` использует `Graviton`.
2. `Electromagnetism` использует `Photon`.
3. `Strong` использует `Gluon`.
4. `Weak` использует собственный переходный канал, не вводимый здесь отдельной именованной сущностью.
5. `Boson` остаётся архитектурным базовым типом таких носителей.
...
7. дедупликация не переносится из `Boundary × Strong` в `Dark`.

Подробный протокольный разбор каналов, включая глюонный октет и его соответствия видам `Field`, задан в [PROTOCOL.md](./PROTOCOL.md).
...
6. `Photon` переносит `State`, `Gluon` изменяет значения `Field`, `Graviton` удерживает скрытый протокол организации; `Impulse` задаёт содержимое такого переноса.
...
2. модули со смыслом зафиксированных состояний, схем или истории размещаются по своей доменной роли,
3. `Dark` не поглощает дедупликацию из `Boundary × Strong`,
4. `Dark` не поглощает исполнение из `Bulk`.

При выделении отдельной файловой проекции `Dark` в неё входят скрытая структура, история схем, зафиксированные состояния, структурированные изменения и историческая непрерывность.
