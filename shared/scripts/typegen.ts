...
export const typegen = async (entrypoint: string, destination: string) => {
  ...
  const cmd = $`dts-bundle-generator --out-file ${destination} --export-referenced-types true --inline-declare-global true --inline-declare-externals true --external-inlines @zavx0z/context --external-inlines @zavx0z/renderer --external-inlines @metafor/template -- ${entrypoint}`
  if (isTTY) {
