...
- `shared/db` владеет плоскими DB-shaped записями, индексами и DB helper API,
- `Dark` владеет экспортом своего object graph в `SharedDbProjection`,
- `Boundary` потребляет эти shared DB-данные через собственный database API и не становится владельцем общего projection-контракта.
...
1. скрытая иерархия,
2. канал `Graviton` как внутренний протокол отношения и локализации внутри скрытой организации,
3. организация схем,
...
2. Он относится к структурному протоколу отношения, локализации и организации.
...
6. `Photon` переносит `State`, `Gluon` изменяет значения обычных `Field`, `Higgs boson` изменяет поля topology, `Graviton` несёт протокол отношения и локализации, `W boson` проводит активный переход, `Z boson` удерживает нейтральную медицию перехода; `Impulse` задаёт содержимое такого переноса.
