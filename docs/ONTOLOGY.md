...
Если в репозитории выделяется `dark/`, то это только явная проекция этой скрытой непрерывности, а не новый runtime-оркестратор.
...
Наряду с этими силами MetaFor различает специальный topology-field слой.
Он не отменяет четыре универсальные силы, а добавляет отдельное различение между данными и топологией.
Topology-fields читаются как Higgs fields, а их изменение переносится через `Higgs boson`.
...
Topology-fields в MetaFor являются Higgs fields.
...
- `enum` всегда выражает topology selection, а не просто ограниченный литеральный набор,
- `array` всегда выражает topology multiplicity / branch expansion, а не обычную коллекцию независимых значений,
...
Когда `array` разворачивается, атом перестаёт читаться как единичное значение и начинает читаться как множественность ветвей: атом становится multiplicity, а не просто контейнером данных.

### State

State — это текущая конфигурация сущности.

- В `Dark` `State` обозначает скрытую непрерывность формы между версиями, а не текущее состояние исполнения и не boundary-runtime snapshot.
...
- В `Dark` `Process` обозначает только историческую линию изменения модели, а не самостоятельный исполняемый процесс и не bulk-runtime execution.
...
Каждый такой подтип принадлежит своей силе или специальному topology-field каналу и не должен смешиваться с другими.
...
- В `Dark` `Gluon` обозначает внутреннее изменение значений ordinary `Field` внутри скрытой структуры.
- В `Boundary` `Gluon` выражает канонически фиксируемое изменение значений ordinary `Field`.
- В `Bulk` `Gluon` выражает применяемое изменение значений ordinary `Field` в проявленной структуре.

При этом `Gluon` относится только к ordinary data-fields.
Он не изменяет topology-fields и не подменяет канал `Higgs boson`.

### Higgs boson

Higgs boson — это подтип `Boson` и канал изменения topology-fields.
...
- В `Dark` `Higgs boson` обозначает скрытую перестройку topology-fields и глобально наблюдаемое изменение topology/gravity-каркаса.
- В `Boundary` `Higgs boson` выражает канонически фиксируемую смену topology selection или branch multiplicity.
...
Подробное протокольное описание силовых каналов задано в [корневом протоколе](./PROTOCOL.md), а различие ordinary fields и topology-fields вынесено в [раздел Strong](./proto/strong.md) и [раздел Higgs](./proto/higgs.md).
...
- topology-fields как скрытые Higgs fields,
- branch selection через `enum`,
- branch multiplicity / expansion через `array`,
- наблюдаемая на уровне домена перестройка topology,
- скрытая реконфигурация topology/gravity-каркаса без превращения `Dark` в runtime-оркестратор.
...
- различение data-field и topology-field на контрактной границе,
...
- чтение structural reconfiguration как наблюдаемого следствия topology-field change.
...
- `Gluon` изменяет значения ordinary `Field`,
...
- `Dark × Higgs` удерживает topology-fields и наблюдает их глобальные структурные следствия,
- `Graviton` выражает скрытый канал `Gravity`,
- `Photon` выражает перенос `State` в `Electromagnetism`,
- `Gluon` выражает изменение значений ordinary `Field` в `Strong`,
- `Higgs boson` выражает изменение topology-fields,
...
- `Dark` наблюдает глобальные следствия `Electromagnetism`, `Higgs field change` и `Gravity`, не становясь runtime-центром их исполнения.
