...
Energy — это домен времени, рантайм-перехода и изменения состояния.
Он возникает после фиксации значения в `Boundary` и не является персистентной базой.
`Energy` получает самодостаточные рантайм-данные через Force/WebSocket и не
...
- `Energy` проводит рантайм-переход и удерживает живое состояние в рантайме,
...
- `Energy` не является персистентным доменом: это рантайм, где протекает вычисление состояния.
- `Bulk` также не является персистентным доменом: это рантайм проявления/проекции.
- `Energy` и `Bulk` не читают `Boundary` напрямую; они получают самодостаточные рантайм-частицы и данные проекции.
...
- В `Energy` `State` вычисляется как рантайм-переход.
...
- В `Energy` `Transition` вычисляется как рантайм-проход.
...
- В `Energy` `Process` участвует в рантайм-условиях перехода.
...
Именно UUID удерживает согласованность одной и той же сущности между `Dark`, `Boundary`, `Energy` и `Bulk`.

- Index — локальный геометрический адрес в `Energy` или `Bulk` во время исполнения.
...
- `Gravity` задаёт рамку отношения и локализации, в которой изменение получает место в `Dark`, `Boundary`, `Energy` и `Bulk`,
...
Явная проекция `dark/` должна удерживать непрерывность `DSL/AST` и контракты нижестоящих доменов, не подменяя `Boundary`, `Energy` или `Bulk`.
...
- как сущности проявляются в `Dark`, `Boundary`, `Energy` и `Bulk`,
