...
Это граница flattening и слой imprint: здесь связность получает фиксируемый отпечаток (`Field`) в форме, пригодной для детерминированного вычисления.
...
В `meta`-декларации `gravity` описывает, как текущая `meta` связана с другими `Wimp` (то есть с другими `meta`-декларациями).
...
Отдельная формализация скрытой связности `Dark`, частиц `Wimp`/`Fuzzy`/`Macho`/`Axion`, а также адресации связности и entanglement вынесена в [TOPOLOGY.md](./TOPOLOGY.md).
...
Явная проекция `dark/` должна удерживать непрерывность `DSL/AST` и downstream contracts, не подменяя `Boundary` или `Bulk`.
