...
В текущей рабочей проекции существуют четыре домена:
...
Имя `Energy` освобождено для следующего слоя: будущей распределённой энергии
исполнения процессов. На текущем этапе `Energy` не является старым доменом
runtime-state.
...
Boundary — это домен предела, фиксации, уплощения и каноникализации.
Это граница уплощения и слой отпечатка: здесь связность получает фиксируемый отпечаток (`Field`) в форме, пригодной для дальнейшего детерминированного рантайм-вычисления.

### Matrix

Matrix — это домен runtime-матрицы состояния, переходов и actor/brane instances.
Он возникает после фиксации значения в `Boundary` и не является персистентной базой.
`Matrix` получает самодостаточные рантайм-данные через Force/WebSocket и не
читает `Boundary`/SQLite как источник данных.

Matrix не исполняет внешний process action. Она вводит actor/brane в
process-bound state, удерживает lock, испускает `photon` при смене состояния,
принимает `w+`/`w-` и применяет результат процесса.

### Energy

Energy зарезервирована для будущего распределённого слоя исполнения процессов.
Energy возникает от state photons Matrix, выбирает подходящий execution runtime
по `env` и `mass`, claim-ит process через `z`, исполняет action в
server/browser/worker/service-worker/main-process/desktop и возвращает
результат через `w+`/`w-`.

### Bulk

Bulk — это домен проявления, объёма, композиции, наблюдаемой формы и пространственной принадлежности.
Это место, где структура получает объёмное проявление и пространственную принадлежность.
В текущей Bulk-проекции `Wimp`/`Fuzzy`/`Macho`/`Axion` проявляются как частицы
Dark Matter с геометрией `Torus`, а обычные поля
`StringField`/`NumberField`/`BooleanField` проявляются как полевые частицы с
геометрией `Sphere`.
`Torus` и `Sphere` являются именами геометрии, а не онтологическими сущностями.
`Row`, `Shell` и `Orbit` не являются именами сущностей Bulk-модели.
...
- `Matrix` проводит рантайм-переход и удерживает живое состояние в рантайме,
- `Energy` в будущем исполняет process actions, рождаемые изменением состояния,
- `Bulk` проявляет объёмную форму и пространственную принадлежность.

Техническая рантайм-проекция этого различения:

- `Dark` может работать совместно с `Boundary`, потому что скрытая связность должна материализоваться в фиксируемую персистентную границу.
- `Matrix` не является персистентным доменом: это рантайм, где протекает вычисление состояния.
- `Bulk` также не является персистентным доменом: это рантайм проявления/проекции.
- `Matrix` и `Bulk` не читают `Boundary` напрямую; они получают самодостаточные рантайм-частицы и данные проекции.
...
- В `Bulk` `Brane` и `Field` существуют как сопряжённые аспекты одной проявленной runtime-структуры.
...
- `matter` может читать topology только через `state`, `enum` и `array` и не должен кодировать topology обычными HTML-узлами или текстом.
...
- В `Boundary` `State` фиксируется как канонический отпечаток.
- В `Matrix` `State` вычисляется как рантайм-переход.
- В `Bulk` `State` проявляется как наблюдаемая runtime-форма.
...
- В `Boundary` `Transition` фиксируется как каноническая возможность перехода.
- В `Matrix` `Transition` вычисляется как рантайм-проход.
- В `Bulk` `Transition` проявляется как видимое изменение формы.
...
- В `Boundary` `Process` присутствует только как фиксируемая декларация/адресация.
- В `Matrix` `Process` участвует в рантайм-условиях перехода.
- В `Bulk` `Process` существует как проявленный след исполнения. Само исполнение process action относится к будущему `Energy`.
...
Он не тождественен самой силе, а обозначает внутреннюю силовую единицу отношения, локализации и организации.
...
- В `Bulk` `W boson` проявляет активный переход как видимое изменение.
...
- В `Bulk` `Z boson` сопровождает проявление как внутреннее сопряжение переходных состояний.
...
- В `Bulk` `Impulse` выражает содержимое применяемого изменения, сопровождающего проявление.

Подробное силовое описание силовых каналов задано в [корневом Force](./FORCE.md), а различие обычных полей и полей topology вынесено в [раздел Strong](./proto/strong.md) и [раздел Higgs](./proto/higgs.md).
...
- проявление process/intent как наблюдаемой формы,
- активный проявленный переход через `W boson`,
- внутреннее сопряжение переходных состояний через `Z boson`,
- проявленный переход,
- отображение результата process, полученного через Force.
...
Именно UUID удерживает согласованность одной и той же сущности между `Dark`, `Boundary`, `Matrix` и `Bulk`.

- Index — локальный геометрический адрес в `Matrix` или `Bulk` во время runtime-проекции.
...
- `Bulk` проявляет доставленное изменение,
- `Gravity` задаёт рамку отношения и локализации, в которой изменение получает место в `Dark`, `Boundary`, `Matrix` и `Bulk`,
- `Dark` наблюдает глобальные следствия `Electromagnetism`, изменения поля Higgs и `Gravity`, не становясь центром их исполнения.

Подробный Force-слой этих соответствий задан в [корневом Force](./FORCE.md), а детальные разборы вынесены в [Gravity](./proto/gravity.md), [Electromagnetism](./proto/electromagnetism.md), [Strong](./proto/strong.md), [Weak](./proto/weak.md) и [Higgs](./proto/higgs.md).
...
Явная проекция `dark/` должна удерживать непрерывность `DSL/AST` и контракты нижестоящих доменов, не подменяя `Boundary`, `Matrix` или `Bulk`.
...
- как сущности проявляются в `Dark`, `Boundary`, `Matrix` и `Bulk`,
