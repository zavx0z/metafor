...
- В `Dark` `State` обозначает скрытую непрерывность формы между версиями, а не текущее состояние исполнения и не snapshot времени исполнения boundary.
...
- В `Dark` `Process` обозначает только историческую линию изменения модели, а не самостоятельный исполняемый процесс и не исполнение времени исполнения bulk.
...
- `Dark` наблюдает глобальные следствия `Electromagnetism`, `Higgs field change` и `Gravity`, не становясь центром их исполнения.
