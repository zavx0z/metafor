...
1. `Dark`, `Boundary` и `Bulk` не должны читаться как внутренние подпакеты одного runtime-модуля,
...
Отдельная формализация скрытого topology-слоя, его локального и глобального уровней, а также его адресации вынесена в [TOPOLOGY.md](./TOPOLOGY.md).
...
5. импортировать `Boundary` или `Bulk` как свои рабочие runtime-зависимости.
...
5. оставаться изолированным доменом, а не внутренним helper-слоем downstream-доменов.
...
4. импортировать `Dark` или `Bulk` как свои прямые рабочие runtime-зависимости.
...
Он строится затем из скрытой topology, но не тождественен topology как отдельному архитектурному объекту.
...
4. импортировать `Dark` или `Boundary` как свои прямые рабочие runtime-зависимости.
...
6. сохранять собственное владение runtime-данными и процессами.
...
5. скрытое удержание topology без превращения `Dark` в runtime-оркестратор.
...
5. В архитектурном чтении `Dark` наблюдает глобальные структурные следствия такого изменения вместе с `Electromagnetism` и `Gravity`, но не превращается в runtime-оркестратор.
...
5. общая скрытая основа не разрешает прямой runtime-import между доменами,
6. до появления протоколов междоменная согласованность доказывается только тестами.

Одновременно `Dark` остаётся скрытым builder/holder topology и gravity-основы.
Он может наблюдать глобальные структурные эффекты, приходящие через `Photon`, `Higgs boson` и `Graviton`, но не должен становиться общим runtime execution center.
