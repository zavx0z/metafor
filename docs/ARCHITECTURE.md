...
- `Matrix` — домен runtime-матрицы состояния, переходов, actor/brane instances, field values и locks,
- `Bulk` — домен проявления, объёма и наблюдаемой формы.

`Energy` зарезервирована для следующего распределённого слоя исполнения
процессов. В текущем коде старый runtime-state домен называется `Matrix`.

`Dark` не тождественен исполняемому `Boundary`.
`Dark` не тождественен проявленному `Bulk`.
...
1. `Dark`, `Boundary`, `Matrix` и `Bulk` не должны читаться как внутренние подпакеты одного модуля времени исполнения,
...
- `Matrix` и `Bulk` не могут импортировать `Boundary` как БД/ORM и не могут читать SQLite.
- `Matrix` и `Bulk` получают данные в реальном времени через Force/WebSocket и ведут собственное состояние/проекцию в рантайме.
...
4. `Boundary`, `Matrix` и `Bulk` читаются как параллельные доменные проекции этого скрытого основания.
...
Онтологически `Boundary`, `Matrix` и `Bulk` имеют общий источник в `Dark`, но технически текущая граница разделяет персистентные и рантайм-слои.
...
- будущий `Energy` будет слушать photons Matrix, claim-ить process через `z` и возвращать `w+`/`w-`;
...
2. читаться как синоним проявленного `Bulk`,
3. превращаться в безымянный слой хранения без собственной онтологической ответственности,
4. поглощать каноникализацию границы или проявление домена `Bulk`,
...
1. знать о process actions будущего `Energy`,
...
5. становиться рантайм-слоем `Matrix` или `Bulk`.

Boundary должен:

1. приводить структуру к канонической форме,
2. удерживать компактное представление,
3. фиксировать данные, из которых `Matrix` может получить рантайм-данные,
...
Bulk — домен проявления, композиции, объёма и наблюдаемой формы.
...
3. проявлять изменение состояния как объёмную runtime-форму,
4. показывать intent/process как наблюдаемый след, не исполняя сам process action,
...
1. проявление process/intent как наблюдаемой формы,
2. активный проявленный переход через `W boson`,
3. внутреннее сопряжение переходных состояний через `Z boson`,
4. visual lifecycle после изменения состояния,
5. отображение результата process, полученного через Force,
6. завершение проявленного такта после обновления Matrix.

Здесь `Bulk` не исполняет process action. Он проявляет переход, intent,
claim/result и итоговое состояние как наблюдаемую runtime-форму.

### Energy × Weak

Роль:

1. будущее распределённое исполнение process action,
2. claim процесса через `Z boson`,
3. проверка `env` и `mass`,
4. запуск action в server/browser/worker/service-worker/main-process/desktop,
5. возврат результата через `W boson` как `w+`/`w-`.

Этот домен ещё не выделен как пакет. Matrix принимает и проверяет result,
а будущий `Energy` будет исполнять process action и возвращать компактный
write-set.
...
3. В `Bulk` она существует как проявленный участник объёмной структуры.
...
5. В `Bulk` `Field` выражает внутреннюю определённость проявленной единицы.
...
3. В `Bulk` `State` проявляется как наблюдаемая форма, но не перестаёт быть частью доменной сущности.
...
3. В `Bulk` `Transition` проявляется как видимое изменение.

### Process

Process в целевой модели исполняется будущим `Energy × Weak`.
Доменное чтение различает его скрытую, граничную, matrix-runtime и проявленную сторону.
...
3. В `Bulk` он проявляет переход как видимое изменение.
...
3. в `Bulk` `Impulse` сопровождает проявление как содержимое применяемого изменения,
...
`UUID` выражает устойчивую идентичность одной и той же сущности и тем самым удерживает согласованность между `Dark`, `Boundary`, `Matrix` и `Bulk`.
`Index` остаётся runtime-геометрией в `Matrix` или `Bulk` и не поднимается в скрытый домен без отдельного онтологического основания.
...
### Проекция `Dark` в `Boundary`, `Matrix` и `Bulk`

`Boundary`, `Matrix` и `Bulk` архитектурно параллельны, потому что укоренены в одном скрытом структурном мире.
...
3. `Matrix` соответствует рантайм-проекции перехода,
...
2. `matrix/` удерживает рантайм-переход и не читает `Boundary` как БД/ORM.
3. `bulk/` удерживает проявленную проекцию/рантайм-форму и не читает `Boundary` как БД/ORM.
4. Первые явные группы владения `Dark` — хранилище структуры связности, поиск path/address, связанное плоское представление и контракты проекций в `Boundary`, `Matrix` и `Bulk`.
...
### Boundary <-> Matrix <-> Bulk

`Boundary`, `Matrix` и `Bulk` архитектурно параллельны.
`Boundary` является персистентной границей, а `Matrix` и `Bulk` являются рантайм-слоями.
`Matrix` и `Bulk` не являются загрузчиками `Boundary` и не читают его БД как скрытый источник.
...
2. `Matrix` получает рантайм-данные и удерживает состояние/переход в рантайме.
...
8. `Boundary` не исполняет process actions будущего `Energy`.
9. `Matrix` и `Bulk` не являются источниками истины для персистентной формы `Boundary`.
10. прямой рабочий импорт `Boundary` в `Matrix`/`Bulk` как DB/ORM-зависимость запрещён.
...
2. `Matrix` вычисляет допустимый рантайм-переход по данным из `Boundary`,
3. междоменное изменение переносится через соответствующий силовой канал,
4. `Bulk` проявляет результат этого перехода,
5. система приходит к следующему согласованному состоянию, совместимому и со скрытой непрерывностью, и с проявленной формой.
...
2. `Matrix` удерживает момент вычисленного перехода.
3. `Bulk` показывает проявленную форму после этого перехода.
4. Lock не должен размывать границу между персистентной фиксацией, рантайм-вычислением и проявлением.

## Оркестраторы

Оркестраторы существуют на доменном уровне в `Boundary`, `Matrix` и `Bulk`.
...
matrix/           # домен Matrix: рантайм времени, перехода и состояния
...
5. `Dark` не поглощает проявление из `Bulk`,
...
Каноникализация границы и проявление домена `Bulk` в неё не входят.
