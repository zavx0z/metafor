[README](../../README.ru.md) | [Протокол](../PROTOCOL.ru.md) | [English](./weak.md) | **Русский**

# Weak

`weak.md` разворачивает протокольное чтение `Weak`.
...
- `Weak` не изменяет значения ordinary `Field` как сила удержания; это делает `Gluon`.
- `Weak` не изменяет topology-fields; это делает `Higgs boson`.
- `Weak` не удерживает скрытую геометрию и адресуемость; это делает `Graviton`.
- `W boson` и `Z boson` вместе завершают слабую симметрию протокольных каналов MetaFor.
