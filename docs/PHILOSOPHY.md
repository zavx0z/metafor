...
MetaFor не объявляет себя физической моделью Вселенной и не переносит
конкретную физическую теорию в код буквально. Он принимает более строгий
инвариант: целостный текущий мир должен иметь каноническую запись на границе, а
каждая проявленная область должна поддерживать собственный store локальной
проекции и parent-child/dependency индексы, не читая скрытые внутренности
другого домена. Целое проявляется не полным снимком, а причинным потоком
минимальных изменений: одна изменённая сущность — один `ForceMessage` с одной
`Particle`.

В этом чтении:

- `Dark` удерживает возможность формы как декларацию и скрытую связность;
- `Boundary` канонизирует декларацию и фиксирует current world;
- `Force` переносит декларации и runtime-изменения отдельными particles;
- `Matrix`, `Energy` и `Bulk` инкрементально наращивают собственные проекции;
- ссылка на чужую БД или память не заменяет данные, пересекающие границу.

Reset противоречит этому чтению: он уничтожает причинную непрерывность мира.
Поэтому cold start, reconnect и обычное изменение используют один и тот же
particle-поток без snapshot, полной очистки или повторного создания целого.

Это позволяет одной модели описывать не только интерфейсы и приложения, но и
инструменты, агентов, память, сознательные и повторяемые подсознательные
процессы, а в дальнейшем и пространства уровня J-space. Эти направления остаются
горизонтом системы; их технический смысл должен появляться через явные
`meta`-декларации, а не через особые скрытые исключения в архитектуре.
...
**Energy** — исполнение процесса и рабочая mass действия.
Это не внешний tool API и не хранилище состояния Matrix.
...
Роль протекания изменения выполняет `Matrix`.
Роль исполнения процесса выполняет `Energy`.
...
В текущей рабочей архитектуре `Matrix` уточняет внутренний рантайм-переход между
...
- `Matrix × Gravity` — рантайм-локализация перехода,
