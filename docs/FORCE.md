...
Остаточный `/field/...` относится только к устаревшему потоку результатов
процессов `z`/`w+`/`w-` до отдельной миграции процессов.
Поля Matrix runtime snapshot с именами `wimpIds` сохраняются только как
...
Имя `Energy` в новых документах относится только к будущему распределённому
исполнителю процессов: он слушает photons Matrix, claim-ит process через `z` и
возвращает результат через `w+`/`w-`. Runtime-state слой называется `Matrix`.
