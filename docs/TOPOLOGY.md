...
Этот документ нужен, чтобы держать согласованность между `meta`, `Dark`, `Boundary`, `Matrix` и `Bulk`.
...
`Dark` не является деревом времени исполнения, не сводится к `AST` и не тождественен итоговой форме `Boundary`/`Matrix`/`Bulk`.
