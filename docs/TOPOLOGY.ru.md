...
`gravity` внутри `meta`-декларации описывает, как текущая `meta` соотнесена и локализована относительно других `meta`.
В чтении `Dark` эти отношения разрешаются через другие `Wimp` в общем скрытом контуре.
...
`gravity` в составе `meta` задаёт локальные отношения и инварианты локализации текущей `meta`.
В этом документе, сфокусированном на `Dark`, они читаются как опорные связи с другими `Wimp`-опорами.
...
9. `gravity` в `meta` описывает отношение и локализацию текущей `meta` относительно других `meta`; в чтении `Dark` это проявляется через другие `Wimp`.
