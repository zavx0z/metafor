 ...
 * @file core/fields.ts
 * @description
 * Fields — арена атомов и витрина их порядка в виде ориентированного дерева.
 *
 * Ключевые свойства:
 * - Хранит **только** экземпляры Atom (никаких «нод»-обёрток).
 ...
 *   - Резервацию места по `id` будущего атома (устойчиво к гонкам).
...
/** Внутренние метаданные атома (не экспонируются). */
interface Meta {
...
/** Описание зарезервированного слота под будущий атом с указанным id. */
...
/**
 * Класс Fields — глобальная арена атомов и упорядоченная витрина их детей.
 *
 * @remarks
 ...
 *   - `reservations`: `id -> Reservation` — зарезервированные слоты под будущие атомы.
 */
export class Fields {
  // -------- singleton --------
  ...
  private requireAtom(id: string): Atom {
    const a = this.atoms.get(id)
    if (!a) throw new Error(`атом не найден: ${id}`)
    return a
  }

  private requireMeta(id: string): Meta {
    const m = this.meta.get(id)
    if (!m) throw new Error(`Метаданные атома отсутствуют: ${id}`)
    return m
  }
  ...
  /**
   * Получить атома по id.
   * @returns Экземпляр атома либо `null`.
   */
  public getAtom(id: string): Atom | null {
    return this.atoms.get(id) ?? null
  }

  /** Проверить наличие атома по id. */
  public has(id: string): boolean {
    return this.atoms.has(id)
  }

  /** Получить всех атомов в системе.
  ...
  /**
   * Построить индекс-путь ("0/1/2") для атома по его текущей позиции.
   * @throws Если атом не найден или витрина рассинхронизирована.
   */
  public getPath(id: string): string {
    if (!this.atoms.has(id)) throw new Error(`атом не найден: ${id}`)
    const indices: number[] = []
    let cur: string | null = id
    while (cur) {
      const m = this.meta.get(cur)
      if (!m) throw new Error(`Метаданные атома отсутствуют: ${cur}`)
      const kids = this.getChildren(m.parent)
      const idx = kids.indexOf(cur)
      if (idx < 0) throw new Error(`Витрина не содержит атома "${cur}" у родителя "${m.parent ?? "root"}"`)
      indices.push(idx)
      cur = m.parent
    }
    indices.reverse()
   ...
   * @param neighborId Существующий атом-ориентир.
  ...
  // -------- создание (принимают Atom) --------

  /**
   * Создать атома и добавить его в конец детей `parentId`.
   * @param parentId id родителя (или null).
   * @param atom Экземпляр атома (должен иметь уникальный `id`).
   */
  public createChildren(parentId: string | null, atom: Atom): void {
    const id = (atom as any).id as string
    if (!id) throw new Error(`У атома отсутствует id`)
    if (this.atoms.has(id)) throw new Error(`атом уже существует: ${id}`)
  ...
  /**
   * Создать атома и вставить его между левым и правым соседями.
   * @param leftId id левого соседа (или null).
   * @param rightId id правого соседа (или null).
   * @param atom создаваемый атом.
   */
  public createBetween(leftId: string | null, rightId: string | null, atom: Atom): void {
    const id = (atom as any).id as string
    if (!id) throw new Error(`У атома отсутствует id`)
    if (this.atoms.has(id)) throw new Error(`атом уже существует: ${id}`)
  ...
  /**
   * Создать атома и вставить его перед соседом.
   * @param neighborId id соседа.
   * @param atom Экземпляр атома.
   */
  public createBefore(neighborId: string, atom: Atom): void {
    const id = atom.id
    if (!id) throw new Error(`У атома отсутствует id`)
    if (this.atoms.has(id)) throw new Error(`атом уже существует: ${id}`)
  ...
  /**
   * Создать атома и вставить его после соседа.
   * @param neighborId id соседа.
   * @param atom Экземпляр атома.
   * @remarks Если справа нет соседа — эквивалентно добавлению в конец.
   */
  public createAfter(neighborId: string, atom: Atom): void {
    const id = (atom as any).id as string
    if (!id) throw new Error(`У атома отсутствует id`)
    if (this.atoms.has(id)) throw new Error(`атом уже существует: ${id}`)
   ...
   */
  public createNode(path: string, atom: Atom): void {
    const id = (atom as any).id as string
    if (!id) throw new Error(`У атома отсутствует id`)
    if (this.atoms.has(id)) throw new Error(`атом уже существует: ${id}`)
    ...
    if (index < children.length) {
      ...
      this.createBefore(neighborId, atom)
      return
    }
    // иначе — добавляем в конец
    this.createChildren(parentId, atom)
  }

  /**
   * Создать атома по заранее рассчитанному лексикографическому ключу.
   * @param parentId Родитель (или null для корня).
   * @param orderKey Лексикографический ключ позиции среди детей родителя.
   * @param atom Экземпляр атома.
   */
  public createWithOrder(parentId: string | null, orderKey: Key, atom: Atom): void {
    const id = (atom as any).id as string
    if (!id) throw new Error(`У атома отсутствует id`)
    if (this.atoms.has(id)) throw new Error(`атом уже существует: ${id}`)
  ...
  /** Отвязать атома от текущего родителя (не удаляя из арены). */
  ...
  /**
   * Перенести детей удаляемого атома на уровень его родителя.
   * @param id Идентификатор удаляемого атома.
   */
   ...
   * @param childId Перемещаемый атом.
  ...
  /**
   * Зарезервировать слот для будущего атома с id `newId`, вычислив позицию
   * по соседу `targetId`: `"before"` или `"after"`.
   */
  public reserveSibling(newId: string, targetId: string, at: "before" | "after" = "after"): void {
    if (this.atoms.has(newId) || this.reservations.has(newId)) {
  ...
  /**
   * Присоединить атом к зарезервированному слоту по его id.
   * Если резервации нет — атом попадёт **в конец корня**.
   */
  public attachReserved(atom: Atom): void {
    const id = (atom as any).id as string
    if (!id) throw new Error(`У атома отсутствует id`)
    if (this.atoms.has(id)) return // уже вставлен

    const r = this.reservations.get(id)
    if (!r) {
      this.createChildren(null, atom)
      return
    }
    this.reservations.delete(id)
