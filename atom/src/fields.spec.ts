...
describe("Fields (лексикографический порядок)", () => {
  ...
  it("createChildren: создаёт атома в корне и добавляет по порядку", () => {
    ...
    expect(fields.getAtom("b")?.id).toBe("b")
  })
  ...
  it("reparentAtom: start/end/after варианты позиционирования", () => {
  ...
  it("unlink: отвязка без удаления из арены", () => {
    ...
    // атом остался
    expect(fields.getAtom("x")?.id).toBe("x")
  })
  ...
  it("remove: удаление одиночного и recursive-поддерева", () => {
    ...
    expect(fields.getAtom("b")).toBeNull()

    // удаляем 'a' рекурсивно (с потомком 'a0')
    fields.remove("a", true)
    expect(fields.getChildren("p")).toEqual([])
    expect(fields.getAtom("a")).toBeNull()
    expect(fields.getAtom("a0")).toBeNull()
  })
  ...
  it("ошибка: createChildren с повторным id", () => {
    ...
    expect(() => fields.createChildren(null, A("dup"))).toThrow("атом уже существует: dup")
  })
  ...
  it("ошибка: appendChild/insert* для неизвестного атома", () => {
    expect(() => fields.appendChild(null, "nope")).toThrow("атом не найден: nope")
    expect(() => fields.insertBetween(null, null, "nope")).toThrow("атом не найден: nope")
  })
  ...
  it("unlink на корне: удаляет из корневой витрины и атом остаётся в арене", () => {
  ...
  it("reparentAtom: 'after' должен принадлежать новому родителю (рекомендуем кидать ошибку)", () => {
    const fields = Fields.get()
    fields.createChildren(null, A("p1"))
    fields.createChildren(null, A("p2"))
    ...
    expect(() => fields.reparentAtom("p1", "m", { at: "after", after: "y" })).toThrow() // добавь явную проверку и понятное сообщение в reparentAtom
  })
  ...
  it("перенос между уровнями: из глубины на корень и обратно с сохранением порядка", () => {
    ...
    fields.reparentAtom("p", "a1", { at: "after", after: "a" })
    expect(fields.getChildren("p")).toEqual(["a", "a1", "b"])
  })
  // it("Atom.createSibling создаёт брата после/перед целевым атомом", () => {
  //   const meta = {
  //     name: "test",
  //     desc: "",
  //     fields: {},
  ...
  //     core: {},
  //   }
  //   const a = Atom.fromSchema({ meta, id: "a" })
  //   const b = Atom.createSibling(a, { meta, id: "b" })
  //   const c = Atom.createSibling(a, { meta, id: "c", at: "before" })
