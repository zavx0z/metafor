...
размера Canvas и циклом кадров; App получает размер через useSpace до
расчёта кадра. Сетка объявлена компонентом Grid из Space. Положение и размеры сцены сохраняют
