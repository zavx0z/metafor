#!/usr/bin/env bun
/**
 * Тонкая обёртка над `@metafor/bun-debug` с дефолтами для dark/:
 *   - inspector URL: ws://127.0.0.1:6499/dark
 *   - state/event/console/command/response файлы кладутся в `dark/debug/`
 *
 * Можно перебить любой env-переменной перед запуском.
 *
 * Запуск: bun run dark/debug/agent-attach.ts
 */
