...
describe("zavx0z/git", () => {
  ...
  describe.skip("zavx0z/git-start", () => {
    test("верхний Dark pipeline проходит только по первой continuation-ветке", async () => {
      ...
      const axion = particles.find(
        (particle): particle is Axion => particle instanceof Axion && dark$.parent.get(particle) === root,
      )
      const fuzzy = particles.find(
        (particle): particle is Fuzzy => particle instanceof Fuzzy && dark$.parent.get(particle) === axion,
      )
      const branchWimps = particles.filter(
      ...
      const firstAxion = particles.find(
        (particle): particle is Axion => particle instanceof Axion && dark$.parent.get(particle) === firstWimp,
      )
      ...
      expect(dark$.parent.get(firstAxion!), "parent Axion первой ветки должен быть самой continuation-веткой").toBe(
        firstWimp,
      )
