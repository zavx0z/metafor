...
function makePlacementId(): string {
  const id = `gp${gravity$.nextPlacementSeq}`
  gravity$.nextPlacementSeq += 1
  return id
}

function makeLinkId(): string {
  const id = `gl${gravity$.nextLinkSeq}`
  gravity$.nextLinkSeq += 1
  return id
}

function makeReferenceId(): string {
  const id = `gr${gravity$.nextReferenceSeq}`
  gravity$.nextReferenceSeq += 1
  return id
}
...
function ensureRootPrefix(meta: string): string {
  const prefix = `/w:${sanitizeMetaSegment(meta)}-${gravity$.rootOccurrenceSeq}`
  gravity$.rootOccurrenceSeq += 1
  return prefix
}
...
function ensureObjectDefinitions(
  ...
  for (const [localObjectId, definition] of Object.entries(fragment.objects as Record<string, any>)) {
    ...
    if (gravity$.objects.has(objectId)) continue
    ...
    gravity$.objects.set(objectId, object)
    strong$.indexObject(objectId, meta)
  }
}

/**
 * Явный object store `@dark/gravity`.
 *
 * Источник истины для world assembly.
 */
export const gravity$: GlobalTopologyStore = {
  fragments: new Map(),
  objects: new Map(),
  placements: new Map(),
  links: new Map(),
  ...
  snapshot(): GlobalTopologySnapshot {
    const strongSnapshot = strong$.snapshotIndexes()
    return {
  ...
  ingestFragment(meta, fragment, options = {} as GlobalTopologyIngestOptions): GlobalTopologyIngestResult {
    ...
    ensureObjectDefinitions(meta, fragment)
    ...
    const rootPrefix = options.parentPlacementId ? null : ensureRootPrefix(meta)
    ...
    for (const localReference of fragment.references) {
      ...
      strong$.indexReference(reference, meta)
    }
    ...
    for (const seed of fragment.entanglementSeeds) {
      ...
      strong$.indexEntanglement(entanglement, meta)
  ...
  getReferencesBySource(metaSource) {
    const referenceIds = strong$.metaSourceLookup.get(metaSource) ?? []
  ...
  getEntanglementByAddress(address) {
    const entanglementId = strong$.entanglementAddressIndex.get(address)
