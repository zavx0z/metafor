...
import {normalizeMetaTemplate} from "../../dark/graph/declaration.ts"
...
import {prepareMatrixBirthFixture} from "../../matrix/tests/shared/fixtures.ts"
...
describe("Graph → Boundary → Matrix Conditions", () => {
  ...
  test("сохраняет весь публичный язык и выполняет один и тот же Transition", async () => {
    ...
    try {
      ...
      const runtime = await import(`../../matrix/matrix.ts?conditions-path=${crypto.randomUUID()}`)
