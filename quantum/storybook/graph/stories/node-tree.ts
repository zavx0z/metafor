...
export function createGraphNodeTreeStory(document: Document): GraphNodeTreeDomStory {
  ...
  const incremented = document.createElement("input")
  ...
  incremented.type = "checkbox"
  incremented.setAttribute("data-control-key", "incremented")
  control.append(incremented, "Изменить runtime count")
  ...
  const onChange = (): void => {
    if (disposed) return
    const next = Object.freeze({incremented: incremented.checked})
    reconcileGraphNodeTree(tree, graphFixture(next.incremented))
    currentArgs = next
    sync()
  }
  incremented.addEventListener("change", onChange)
  sync()
  ...
  return Object.freeze({
    ...
    dispose() {
      if (disposed) return
      disposed = true
      incremented.removeEventListener("change", onChange)
      tree.dispose()
    },
  })
}
