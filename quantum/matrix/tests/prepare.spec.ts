...
function getBraneSharedBlockIds(store: MatrixData, braneIndex: number) {
  const brane = store.branes[braneIndex]
  if (!brane) {
    return []
  }
...
function getSharedBlockValues(store: MatrixData, blockIndex: number) {
...
function getBraneStateTransitions(store: MatrixData, braneIndex: number, stateIndex: number) {
  const brane = store.branes[braneIndex]
  ...
  return store.transitions
    ...
    .map((transition: MatrixTransitionRecord) => ({
...
describe("prepareMatrixData — подготовка canonical JS store", () => {
  test("Matrix flattening должен переводить nested conditions в parsed checks", () => {
    ...
    const flattened = flattenMatrixData(data)
