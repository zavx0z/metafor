/**
Dark checkpoint-представление валидированного complete Graph.

Canonical bytes и forward JSON Patch являются replay artifacts, а не live
mutation language, public Graph delta или второй causal history.
...
/** Валидированный Graph с deterministic bytes и lowercase SHA-256. */
export type CanonicalGraphProjection = {
  value: Graph
  bytes: Uint8Array
  sha256: string
}
/** Fail-closed ошибка canonicalization либо применения patch. */
...
/** Преобразует один complete Graph в deterministic checkpoint bytes. */
...
/** Выводит deterministic forward operations; изменённый массив заменяется целиком. */
...
/** Применяет checkpoint patch к detached copy и валидирует полный результат. */
