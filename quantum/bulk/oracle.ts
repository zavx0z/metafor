...
export type BulkOracleState = "created" | "loading" | "prepared" | "ready" | "error" | "stopped"
...
export class BulkOracle {
  ...
  async onServerStarted(peer?: Pick<OracleRpcPeer, "call">): Promise<void> {
    if (this.#state !== "created") throw new Error(`Bulk Oracle cannot start from state: ${this.#state}`)
    this.#state = "loading"
    try {
      if (peer !== undefined) {
  ...
  acceptImpulse(message: ForceMessage): void {
    if (this.#state !== "ready") {
      throw new Error(`Bulk Oracle cannot apply a Store update from state: ${this.#state}`)
    }
