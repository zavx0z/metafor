...
## Согласованный начальный срез

Начальные State, Projection, replay и составной Graph snapshot читаются в той
же последовательной границе, что изменения мира. Каждое такое чтение видит
целиком состояние до соседнего изменения или целиком состояние после него и не
может собрать один ответ из строк разных моментов.
...
Authoring-перенос root WIMP Matter между двумя Meta приходит одной
`inflaton/move` Particle. Её `from` называет исходную декларацию как
`<wimp-src>#<localId>`, потому что Dark не читает Boundary и её SQLite row IDs.
Boundary разрешает эту identity внутри той же транзакции, переносит существующую
строку `matter_particle` и, когда исходная и целевая runtime placement имеют
однозначное соответствие в одном scope, перепривязывает существующий Atom без
смены его ID. Неоднозначный перенос отклоняется целиком.
...
## Каноническая очередь Reaction

Каждый полученный от Matrix enqueue подтверждённого State и точной Reaction
relation становится отдельной durable записью Boundary. Boundary возвращает
Matrix точный порядок записей одного target Atom, но не решает, какую из них
начать: Matrix отдельно запускает только первый элемент.

Новая запись остаётся ожидающей, не содержит снимок Fields и ещё не разрешает
объявленный доступ к Mass. Boundary снимает Fields и проверяет доступные Mass
keys только при подтверждённом Matrix запуске первого элемента, после terminal
result всех более ранних элементов. Поэтому
последовательные Reaction читают результаты друг друга, а повторная доставка
того же события и relation возвращает уже существующую запись без второго
исполнения.

Согласованный начальный снимок передаёт Matrix все незавершённые элементы в
точном порядке. Не выбранное Energy execution после холодного рождения снова
предлагается с той же identity. Если Energy уже была выбрана, прежний action мог
успеть записать Mass, поэтому Boundary переводит execution в `superseded` без
повторного запуска и без отката Mass. Изменение точной relation или выход target
Atom из всех активных States переводит в `superseded` как выполняющийся, так и
ещё ожидающий элемент.
