...
WebGPU capture. Ordinary Cosmos development uses `metafor-dev.sh` without
Inspector.
...
2. Run `scripts/metafor-dev.sh start <checkout>` only when the managed contour
   is stopped. Do not launch another Chrome or Cosmos process.
...
1. Keep the clean Cosmos target open.
...
8. Close only the temporary instrumented page. Leave the clean target, CDP
   Chrome, and requested Cosmos process in their agreed state.
