...
Bun.serve<ForceSocketData>({
  ...
  routes: {
    "/energy/runtime": {
      GET: () => {
        const db = new Database(BOUNDARY_PATH, {readonly: true, create: false})
        try {
          ...
          const all = <T>(sql: string): T[] => db.query(sql).all() as T[]

          const actors = all<{uuid: string; wimp: string; position: number}>(
            "SELECT uuid, wimp, position FROM actor ORDER BY rowid",
          )
          const fields = all<{
            uuid: string
            wimp: string
            ...
            required: number
            label: string | null
          }>("SELECT uuid, wimp, key, type, required, label FROM field ORDER BY wimp, rowid")
          const enumVariants = all<{uuid: string; field: string; position: number; itemValue: string}>(
            "SELECT uuid, field, position, item_value AS itemValue FROM field_enum_variant ORDER BY field, position",
          )
          const states = all<{uuid: string; wimp: string; name: string; position: number}>(
            "SELECT uuid, wimp, name, position FROM state ORDER BY wimp, position",
          )
          const transitions = all<{uuid: string; fromState: string; toState: string; position: number}>(
            "SELECT uuid, from_state AS fromState, to_state AS toState, position FROM transition ORDER BY from_state, position",
          )
          const conditions = all<{uuid: string; transition: string; field: string; position: number}>(
            "SELECT uuid, transition, field, position FROM condition ORDER BY transition, position",
          )
          const predicates = all<{
            uuid: string
            condition: string
          ...
          const processes = all<{uuid: string; wimp: string; key: string}>(
            "SELECT uuid, wimp, key FROM process ORDER BY wimp, rowid",
          )
          const actorValues = all<{actor: string; field: string; value: string}>(
            "SELECT actor, field, value FROM actor_value ORDER BY actor, field",
          )
          const actorStates = all<{actor: string; metaState: string | null}>(
          ...
          const valueListItems = all<{value: string; position: number; itemValue: string}>(
          ...
          const decodeValue = (valueId: string | undefined, field: (typeof fields)[number]): unknown => {
          ...
          const energyField = (field: (typeof fields)[number]) => {
            if (field.type === "number") return {type: fieldType.F32}
            if (field.type === "boolean") return {type: fieldType.BOOL}
            if (field.type === "array") return {type: fieldType.ARRAY_PTR, elementType: "string" as const}
            if (field.type === "enum") {
          ...
          const predicateValue = (predicate: (typeof predicates)[number]): unknown => {
          ...
          const mergePredicate = (target: Record<string, unknown>, predicate: (typeof predicates)[number]): void => {
          ...
          const dataFields: Array<{type: number; elementType?: "string"; enum?: unknown[]}> = []
          const branes: Array<{values: Array<[number, unknown]>; state: number; collapses: Array<Array<[number, Record<number, unknown>] | null>>}> = []
