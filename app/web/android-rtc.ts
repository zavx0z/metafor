...
export function createAndroidRtcClient(opts: AndroidRtcClientOpts): AndroidRtcClient {
	...
	async function handleSignal(signal: AndroidRtcSignal): Promise<void> {
		if (signal.type === "hello") {
			...
			for (const remotePeerId of signal.peers) {
				if (isAndroidRtcSenderPeer(remotePeerId)) void createPeer(remotePeerId, true)
			}
			return
		}
		if (signal.type === "peer-joined") {
			if (isAndroidRtcSenderPeer(signal.peerId)) void createPeer(signal.peerId, true)
			return
		}
		...
		if (!isAndroidRtcSenderPeer(signal.from)) return
	...
	function closeAllPeers(): void {
		for (const item of [...peers.keys()]) closePeer(item)
	}
