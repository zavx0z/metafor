...
- Если нужно читать, открывать или редактировать активные файлы внутри текущей среды интерпретатора, использовать единый API интерпретатора: `/processes/:id/tools` с `source.read`, `source.open`, `source.write` или `source.apply_patch`.
