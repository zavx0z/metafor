...
- `app/web/server.ts` поднимает `dark/server`, открывает `Boundary` по `BOUNDARY_PATH`, материализует `zavx0z/git`, получает снимок `boundary.bulkRuntime()` и отдаёт браузеру готовые строки мира.
