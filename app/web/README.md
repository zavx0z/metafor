...
- `app/web/server.ts` поднимает server-side `boundary`, `dark` и `bulk` worker-ы поверх общего file-backed SQLite backend (`app/web/tmp/metafor-app.sqlite`) c `WAL`, зеркалит единый protocol channel в UI, принимает входные patches с `part: "gluon" | "higgs"` через `/ws` и исполняет `Bulk × Weak` процессы по `Photon -> +Z/-Z/W`
