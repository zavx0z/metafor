...
Документ разворачивает [CURRENT_PLAN.md](./CURRENT_PLAN.md) только для домена `Boundary`.
...
2. `boundary/store.ts` остаётся единственным источником истины только для boundary-домена, а не для source graph.
...
1. Оставить `boundary/boundary.ts` boundary-доменным оркестратором поверх dark-provided structure.
2. Оставить `boundary/store.ts` и `boundary/store.t.ts` единственным источником истины boundary-домена.
...
4. Держать `boundary/weak/runtime/store.ts` локальной производной runtime-структурой.
...
2. Перенести dump-проекцию снимка внутрь `boundary/strong/dump/` как часть `Boundary × Strong`.
...
1. Оставить `boundary/index.ts` тонким публичным входом boundary-домена.
...
4. Синхронизировать `boundary/README.md` и `boundary/strong/dump/README.md` только после фактического переноса модулей.
...
2. `boundary/store.ts` остаётся источником истины только для boundary-представления.
