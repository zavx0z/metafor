 ...
 * `none` < `effects` < `appearance` < `relations` < `geometry` < `structure`.
 * Correctness always wins over locality — a scope
...
export const classifyVisualInvalidation = (
  ...
  switch (change.facet) {
    // Nothing semantic moved: the held declarative scene is still current.
    case "none":
      return "none"
    case "effect":
      return "effects"
