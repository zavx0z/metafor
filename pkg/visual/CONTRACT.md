...
- `centered-nested` определяет общность Fields только по materialized
  ...
  размер маркера определяется уровнем этого предка. Совпадение имени либо
  payload не создаёт общности; computed Matter binding с новым `Value`
  остаётся независимым.
- Не разделяемые с потомками Fields корневого Atom образуют центральное ядро
  по общему закону плоской псевдоокружности. Единые маркеры общих величин
  родителя и вложенных Atom образуют следующую орбиту своего верхнего общего
  предка. Независимые от родителя Fields внутреннего Atom образуют следующую
  внешнюю орбиту ядра. Тот же порядок рекурсивно повторяется для каждой
  следующей Matter-глубины.
- Поверхностный зазор от уже занятого ядра до каждой следующей Field-орбиты
  равен одному полному диаметру Field уровня этой орбиты. Первая орбита
  сохраняет этот минимальный радиус и не расширяется из-за количества Fields.
  Если её окружности недостаточно для непересекающихся полных маркеров,
  создаются следующие концентрические орбиты той же группы без нового
  поверхностного зазора: соседние орбиты разделяет только сумма радиусов их
  Field-маркеров. Маркеры детерминированно распределяются между созданными
  орбитами пропорционально их геометрической вместимости; Field-маркеры не
  уменьшаются.
