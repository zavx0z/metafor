...
- Поверхностный зазор от уже занятого ядра до каждой следующей Field-орбиты
  ...
  создаются следующие концентрические орбиты с тем же поверхностным зазором.
  Маркеры детерминированно распределяются между созданными орбитами
  пропорционально их геометрической вместимости; Field-маркеры не уменьшаются.
