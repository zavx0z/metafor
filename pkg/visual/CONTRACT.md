...
- Field-ядро любого Torus использует одну детерминированную плоскую
  псевдоокружность: первый Field расположен точно в центре, остальные Fields
  равномерно распределены по одной окружности без пересечений. Радиус этой
  окружности выводится напрямую из количества и диаметра Fields. Радиус самого
  Field задаётся только уровнем вложения и не зависит от их количества,
  размера отверстия, камеры либо viewport.
