...
const applyStory = (): void => {
  ...
  description.textContent = selectedLayout?.slug === OutsideIn.slug
    ...
    : selectedLayout?.slug === CenteredNested.slug
      ? `${CenteredNested.description} Fields с одним materialized Value отображаются одним маркером у их верхнего общего предка. Единственный зазор в один диаметр отделяет центральные личные Fields от первой общей орбиты; дополнительные shared-орбиты идут без повторного зазора и получают маркеры пропорционально своей вместимости. Независимые Fields корня остаются в центральной псевдоокружности, а частные Fields внутренних Atom последовательно раскрываются наружу.`
