...
const applyStory = (): void => {
  ...
  description.textContent = selectedLayout?.slug === OutsideIn.slug
    ...
    : selectedLayout?.slug === CenteredNested.slug
      ? `${CenteredNested.description} Fields с одним materialized Value отображаются одним маркером у их верхнего общего предка. Первая орбита сохраняет минимальный радиус после зазора в один диаметр; если маркеры не помещаются, они пропорционально распределяются по дополнительным концентрическим орбитам. Независимые Fields корня остаются в центральной псевдоокружности, а частные Fields внутренних Atom последовательно раскрываются наружу с тем же зазором своего уровня.`
