...
const applyStory = (): void => {
  ...
  description.textContent = selectedLayout?.slug === OutsideIn.slug
    ...
    : selectedLayout?.slug === CenteredNested.slug
      ? `${CenteredNested.description} Fields с одним materialized Value отображаются одним маркером у их верхнего общего предка. Личные Fields корня остаются в центральной псевдоокружности; после единственного зазора глубже вложенные владельцы располагаются ближе к центру, а Fields одного глубокого владельца кластеризуются рядом независимо от shared/private роли. Дополнительные орбиты идут без повторного зазора и получают маркеры пропорционально своей вместимости.`
