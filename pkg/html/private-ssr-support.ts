...
import type {DirectiveClass, DirectiveResult, PartInfo} from "./types/directives.ts"
 ...
 *
 * В настоящее время мы не создаем минифицированную сборку кода @quantum/html-ssr. Чтобы
 ...
 * к приватным полям этих членов), а затем повторно экспортируем их для использования в @quantum/html-ssr.
 * Это позволяет @quantum/html-ssr оставаться независимым от того, используется ли клиентский код
...
export const _$LH = {
  ...
  patchDirectiveResolve: (
    ...
    if (directiveClass.prototype._$resolve !== resolveOverrideFn) {
      ...
      // каким-то образом и минифицированный, и неминифицированный код @quantum прошел через этот
      // путь выполнения. Это возможно, так как @quantum-labs/ssr содержит свой собственный @quantum/html
      // модуль как зависимость для серверного рендеринга клиентского @quantum кода. Если
      // клиент содержит несколько дублирующихся @quantum модулей с минифицированными и
  ...
  getAttributePartCommittedValue: (part: AttributePart, value: unknown, index: number | undefined) => {
    ...
    // метод запускается только на `AttributePart`, созданных @quantum-ssr, использующих ту же
