...
В компоненте @quantum/html это означает всякий раз, когда render вызывается метод.
...
@quantum/html может отображать почти все примитивные значения и преобразует их в строки при интерполяции в текстовом содержимом.
...
@quantum/html предоставляет несколько специальных значений-сентинелов, которые можно использовать в дочерних выражениях.
...
@quantum/html также предоставляет директиву `ifDefined`, которая упрощает использование `value ?? nothing`:
...
Обратите внимание, что имя свойства в этом примере — `listItems` — написано в смешанном регистре (camelCase). Хотя HTML-атрибуты не чувствительны к регистру, @quantum/html сохраняет регистр для имён свойств при обработке шаблона.
...
В @quantum/html-компоненте обработчик события автоматически привязывается к компоненту, поэтому вы можете использовать значение `this` внутри обработчика для ссылки на экземпляр компонента.
...
Шаблоны @quantum/html должны быть корректным HTML, поскольку они парсятся встроенным HTML-парсером браузера перед интерполяцией значений. Следуйте этим правилам:
...
Выражения не могут использоваться там, где должны быть имена тегов или атрибутов. @quantum/html не поддерживает динамическое обновление таких значений и выдаст ошибку в режиме разработки:
...
@quantum/html может рендерить контент в `<textarea>`, но редактирование содержимого приведет к разрыву ссылок на DOM, используемых для динамического обновления. Вместо этого используйте привязку к свойству `.value`:
...
@quantum/html не обновляет выражения в комментариях. Такие выражения будут отрендерены как строка токена Lit, но это не повлияет на последующие выражения. Поэтому закомментировать блоки HTML, содержащие выражения, безопасно во время
разработки:
...
Статические выражения возвращают специальные значения, которые интерполируются в шаблон до того, как он обрабатывается @quantum/html как HTML. Поскольку они становятся частью статического HTML-шаблона, их можно размещать в любом
месте шаблона, даже там, где обычные выражения запрещены, например в именах атрибутов или тегов.

Для использования статических выражений необходимо импортировать специальную версию функций шаблонов `html` или `svg` из модуля `@quantum/html/static-html.js`:

```javascript
import {html, literal} from '@quantum/html/static-html.js';
```

Этот модуль предоставляет функции `html` и `svg`, поддерживающие статические выражения, которые следует использовать вместо стандартных версий из модуля `@quantum/html`. Используйте функцию `literal` для создания статических
выражений.
...
```javascript
import {LitElement} from '@quantum/html';
import {html, literal} from '@quantum/html/static-html.js';
...
Изменение значений статических выражений требует больших затрат. Если значения `literal` часто меняются, @quantum/html создаёт новый шаблон, что неэффективно. Это приводит к полной перерисовке DOM и увеличению использования памяти,
...
После интерполяции статических значений шаблон должен быть корректно сформирован, как обычные шаблоны @quantum/html. В противном случае динамические выражения могут работать некорректно. Подробнее см.
...
```javascript
import {html, unsafeStatic} from '@quantum/html/static-html.js';
```
