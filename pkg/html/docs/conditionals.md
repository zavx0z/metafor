...
@quantum/html позволяет использовать стандартные JavaScript-конструкции управления потоком, такие как [условные операторы](https://developer.mozilla.org/ru/docs/Web/JavaScript/Reference/Operators/Conditional_Operator), вызовы
...
Значения `undefined`, `null`, пустая строка (`''`) и специальное значение `nothing` из @quantum/html не создают узлы DOM:
