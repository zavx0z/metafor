...
@quantum/html также предоставляет директиву `repeat` для более эффективного создания определённых типов динамических списков.

---

## Рендеринг массивов

Когда выражение в позиции дочернего элемента возвращает массив или итерируемый объект, @quantum/html рендерит все элементы массива:
...
Данное выражение возвращает массив объектов `TemplateResult`. @quantum/html рендерит массив или итерируемый объект, содержащий подшаблоны и другие значения.
...
1. **Используйте `repeat`, если:**
   ...
   В этом примере состояние чекбоксов не контролируется шаблоном. Если вы измените порядок списка, @quantum/html обновит имена, но состояние чекбоксов останется прежним.
