...
Директивы — это функции, которые могут расширять возможности @quantum/html, настраивая, как выражение шаблона рендерится. Директивы полезны и мощны, поскольку они могут иметь состояние, обращаться к DOM, получать уведомления при
...
@quantum/html поставляется с рядом встроенных директив таких как `repeat()` и `cache()`. Пользователи также могут писать свои
...
```javascript
import {Directive, directive} from "@quantum/html/directive.js"
...
Когда этот шаблон оценивается, функция директивы (`hello()`) возвращает объект `DirectiveResult`, который инструктирует @quantum/html создать или обновить экземпляр класса директивы (`HelloDirective`). Затем @quantum/html вызывает методы
...
Когда @quantum/html впервые сталкивается с `DirectiveResult` в выражении, он создаст экземпляр соответствующего класса директивы (что вызовет конструктор директивы и любые инициализаторы полей класса):
...
Ваш метод `update()` должен возвращать что-то, что @quantum/html может отрендерить, или специальное значение `noChange`, если повторный рендеринг не требуется. Обратный вызов `update()` достаточно гибок, но типичные использования
включают:

- Чтение данных из DOM и использование их для генерации значения для рендеринга.
- Императивное обновление DOM с использованием ссылки `element` или `parentNode` на объекте `Part`. В этом случае `update()` обычно возвращает `noChange`, указывая, что @quantum/html не нужно предпринимать никаких дальнейших
...
Хотя обратный вызов `update()` более мощный, чем `render()`, существует важное различие: при использовании пакета `@quantum/html/ssr` для серверного рендеринга (SSR) **только** метод `render()` вызывается на сервере. Чтобы
...
Иногда директива может не иметь ничего нового для рендеринга. Вы сигнализируете об этом, возвращая `noChange` из метода `update()` или `render()`. Это отличается от возврата `undefined`, что заставляет @quantum/html очистить `Part`,
...
```typescript
import {Directive} from "@quantum/html/directive.js"
import {noChange} from "@quantum/html"
