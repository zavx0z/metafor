...
import type {Brand, StaticValue} from "./types/static.ts"
...
/** Оборачивает тег шаблона @quantum/html (`html` или `svg`) для добавления поддержки статических значений. */
    ...
    (strings: TemplateStringsArray, ...values: unknown[]): TemplateResult => {
      ...
      if (hasStatics) {
        ...
        if (strings === undefined) {
          ...
          // безопасности @quantum/html, позволяя злоумышленнику выполнять произвольный
 ...
 * Включает поддержку статических значений из `@quantum/html/static.js`. */
