...
- `browser_chat.send` accepts `attachmentPaths?: string[]` for DeepSeek. The runtime asks the host to put those server-local files into the provider file input before sending the message. Image-only sends are valid when `message`/`text` is empty and at least one attachment path is present; the runtime does not invent a caption.
