...
- Browser Agent Chat UI имеет provider sessions `Qwen` и `DeepSeek`. Каждая session держит отдельные history, draft, attachments, transport state и tool loop state; один composer/editor всегда работает с active session. Session state хранится в browser `localStorage`, поэтому active tab, drafts, attachments, messages, DeepSeek mode/deep-thinking и tool loop controls переживают reload UI; live busy/canSend дополнительно гидратируется через `browser_chat.read`.
...
- Голосовые wake-команды `Квин`/`Qwen` и `Дипсик`/`DeepSeek` открывают Browser Agent Chat, переключают active provider session, активируют соответствующую Chrome-вкладку и назначают Browser Agent composer текущим voice target; эти слова не должны попадать в draft как пользовательское сообщение.
