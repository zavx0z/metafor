...
- Голосовые wake-команды `Завхоз`/`Запхоз`/`Метафор` возвращают текущий voice target в Codex message host composer. Wake-команды `Квин`/`Qwen` и `Дипсик`/`DeepSeek` открывают Browser Agent Chat, переключают active provider session, активируют соответствующую Chrome-вкладку и назначают Browser Agent composer текущим voice target; эти слова не должны попадать в draft как пользовательское сообщение.
