...
Codex message и Browser Agent message используют один общий HUD composer с target-кнопками `Codex`/`Qwen`/`DeepSeek`, но остаются разными transport:

- Target `Codex` отправляет текст, голос и image attachment paths в host PTY/Codex CLI через terminal transport.
- Targets `Qwen`/`DeepSeek` отправляют текст, голос и image attachment paths в уже открытый browser chat через Chrome DevTools DOM bridge.
...
- Голосовые wake-команды `Завхоз`/`Запхоз`/`Метафор` возвращают текущий voice target в общий composer с target `Codex`. Wake-команды `Квин`/`Qwen` и `Дипсик`/`DeepSeek` открывают Browser Agent Chat, переключают active provider session и composer target, активируют соответствующую Chrome-вкладку и назначают Browser Agent target текущим voice target; эти слова не должны попадать в draft как пользовательское сообщение.
