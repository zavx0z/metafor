...
- Browser Agent Chat UI имеет provider sessions `Qwen` и `DeepSeek`. Каждая session держит отдельные history, draft, attachments, transport state и tool loop state; один composer/editor всегда работает с active session.
