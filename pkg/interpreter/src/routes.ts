...
const routeIndex = [
  ...
  {method: "GET", path: "/remote-desktop/health", description: "proxy к локальному Electron remote desktop host health; включает WebRTC state и fallback snapshot diagnostics"},
  ...
  {method: "GET", path: "/remote-desktop/rtc/state", description: "state WebRTC sender внутри Electron host"},
  {method: "POST", path: "/remote-desktop/rtc/restart", description: "перезапустить WebRTC sender/capture window Electron host"},
  {method: "GET", path: "/remote-desktop/snapshot", description: "fallback PNG snapshot через Electron host; не основной realtime display"},
  {method: "POST", path: "/remote-desktop/input", description: "fallback/control input в Electron host или configured desktop input adapter"},
  ...
  {method: "POST", path: "/remote-desktop/browser/open", description: "{url} — открыть URL в Electron browser window или configured browser adapter"},
