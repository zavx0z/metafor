...
describe("workspaceFilesPayload", () => {
  test("uses launch cwd as root and follows imported workspace files", () => {
    ...
    expect(payload.files.map((file) => file.path)).toContain("pkg/protocol/index.ts")
  ...
  test("keeps independent import graphs for different processes", () => {
    ...
    expect(dark.files.map((file) => file.path)).toContain("pkg/protocol/index.ts")
    ...
    expect(interpreter.files.map((file) => file.path)).not.toContain("pkg/protocol/index.ts")
  })
  ...
  test("filters imported catalog by query", () => {
    ...
    expect(payload.files.map((file) => file.path)).toEqual(["pkg/protocol/index.ts"])
  })
})
...
function testWorkspace(): string {
  ...
  writeFile(cwd, "dark/package.json", JSON.stringify({
    name: "@metafor/dark",
    dependencies: {"@shared/protocol": "workspace:*", store: "workspace:*"},
  }))
  writeFile(cwd, "dark/server.spec.ts", "import {GRAVITY_BROADCAST_CHANNEL} from '@shared/protocol'\nawait import('./server.ts')\ntest('dark', () => GRAVITY_BROADCAST_CHANNEL)")
  writeFile(cwd, "dark/server.ts", "import {GRAVITY_BROADCAST_CHANNEL} from '@shared/protocol'\nimport {MetaFor} from '..'\nimport {open} from 'store/server'\nexport {GRAVITY_BROADCAST_CHANNEL, MetaFor, open}")
  writeFile(cwd, "dark/weak/index.ts", "export {}")
  writeFile(cwd, "pkg/protocol/package.json", JSON.stringify({
    name: "@shared/protocol",
    exports: {".": "./index.ts"},
  }))
  writeFile(cwd, "pkg/protocol/index.ts", "export const GRAVITY_BROADCAST_CHANNEL = 'metafor.gravity'")
