 ...
 * этот process через process.action в POST /tools).
 *
...
export class TargetSupervisor {
  ...
  start(options: {
    ...
    if (this.#state === "starting" || this.#state === "running") {
      throw new Error(`модуль уже запущен (pid=${this.#pid}); сначала останови process через process.action в POST /tools`)
    }
