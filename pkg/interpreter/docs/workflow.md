...
Display процесса, через который пришел API patch, обновляется даже если прежний
editor buffer был dirty или process уже упал: agent/source API является
источником нового snapshot. Другие display с локальным dirty state не
перетираются автоматически; пока идет сохранение, авто-переход тоже
пропускается.

## SQLite HUD

SQLite HUD открывается из CLI входа `.sqlite` или через `POST /sqlite/open`. Панель можно свернуть и развернуть через `/hud/sqlite/dock|show|toggle`; свернутое состояние живет в HUD dock как отдельная вкладка, наравне с Plan и host terminal.
