...
Остановка на первой строке не является обычным поведением restart. Она нужна только для явного интерактивного запуска с `pauseOnStart: true`, который переводится в `--inspect-brk=<protocol-url>`. Для короткого модуля или теста, где нужно успеть поставить breakpoint до выполнения, использовать REST `POST /processes` или `POST /processes/:id/action` с явным `pauseOnStart: true`.
