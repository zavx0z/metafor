...
Browser-страница сейчас является host-слоем для canvas/input/WebSocket. Это не продуктовая граница интерпретатора: для XR тот же module-scoped state должен рендериться как `UIDisplay`-контент в общем `Space`.
...
Это нужно, чтобы короткий модуль или тест не завершался до того, как человек успел поставить breakpoint. Если нужен запуск без начальной остановки, использовать REST `POST /modules/:id/run` с явным `pauseOnStart: false`.
