...
6. Каждый semantic edge сохраняет собственный ID и exact endpoints, но
   связанные edges, имеющие общий source-node или общий target-node, могут
   использовать один сгенерированный ортогональный trunk. Совпавший участок
...
7. Несвязанные edges не получают права на overlap. Между их параллельными
