              ...
              items: Array.from({length: columns}, (_, column) => {
                ...
                return socket === undefined ? {width: "1fr" as const, height: rowH, draw: () => {}} : {
                  ...
                  draw: (cellX: number, cellY: number, cellW: number, cellH: number) => flexRow({
                    ...
                    alignItems: "center",
                    items: [
                      {width: 16, height: 16, draw: (slotX, slotY, slotW, slotH) => blenderSocketRenderer.render({host: this, entry: {socket, side: socket.direction === "output" ? "right" : "left", center: {x: slotX + slotW / 2, y: slotY + slotH / 2}}, scale: 1, selected: false, nodeId: "socket-catalog"})},
