...
export class SocketCatalogSurface extends UiSurface {
  ...
  #drawSocketCell(socket: BlenderSocket, label: string, selected: boolean, x: number, y: number, w: number, h: number): void {
    ...
    flexRow({
      ...
      alignItems: "center",
      items: [
        {width: 22, height: 22, draw: (slotX, slotY, slotW, slotH) => blenderSocketRenderer.render({host: this, entry: {socket, side: socket.direction === "output" ? "right" : "left", center: {x: slotX + slotW / 2, y: slotY + slotH / 2}}, scale: 1.2, selected, nodeId: "socket-catalog"})},
