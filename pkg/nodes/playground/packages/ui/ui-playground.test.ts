...
const nodesPlaygroundRoot = fileURLToPath(new URL("../..", import.meta.url))
...
describe("Blender-like Node component playground", () => {
  test("imports universal fields only inside Node composition", async () => {
    ...
    const surfaces = await Bun.file(join(playgroundRoot, "surfaces/reference-surfaces.ts")).text()
  ...
  test("opens package, Parameter and Socket prefixes before exact detail stories", () => {
    ...
    expect(nodePlaygroundCatalog("parameter/text/field", new Set(["components"])).map(({group}) => group?.collapsed === true)).toEqual([
      false,
      false,
      false,
      true,
    ...
    expect(nodePlaygroundWorkbenchStoryRoute("")).toBe("parameter/text/field")
    expect(nodePlaygroundWorkbenchStoryRoute("node-editor")).toBe("node-editor/scene/default")
    expect(nodePlaygroundWorkbenchStoryRoute("parameter")).toBe("parameter/text/field")
    expect(nodePlaygroundWorkbenchStoryRoute("parameter/text")).toBe("parameter/text/field")
    expect(nodePlaygroundWorkbenchStoryRoute("parameter/vector")).toBe("parameter/vector/field")
    ...
    expect(nodePlaygroundGroup("parameter/text/field")).toBe("parameter")
    ...
    expect(nodePlaygroundSections("parameter").map(({id}) => id)).toEqual([...NODE_PARAMETER_FIELD_KINDS])
    expect(nodePlaygroundDockItems("parameter/text").map(({id}) => id)).toEqual([...NODE_PARAMETER_VARIANTS])
    expect(nodePlaygroundDockItems("parameter/collection").map(({id}) => id)).toEqual([...NODE_PARAMETER_VARIANTS])
  ...
  test("uses one Card-free WebGPU component graph and disables HMR", async () => {
    const server = await Bun.file(join(nodesPlaygroundRoot, "server.ts")).text()
    const pageRegistry = await Bun.file(join(nodesPlaygroundRoot, "server/page-registry.ts")).text()
    const surfaces = await Bun.file(join(playgroundRoot, "surfaces/reference-surfaces.ts")).text()
    expect(server).toContain("startPlaygroundHubServer")
    expect(pageRegistry).toContain("ui: pageFiles({")
    expect(pageRegistry).toContain('canvasId: "nodes-playground-canvas"')
    expect(server).toContain("/assets/ui/blender-4.5.5-reference.png")
    expect(surfaces).toContain("/assets/ui/blender-4.5.5-reference.png")
    expect(`${server}\n${surfaces}`).not.toContain(["", "node-system-dev"].join("/"))
    expect(await Bun.file(join(playgroundRoot, "blender-4.5.5-reference.png")).exists()).toBeTrue()
    const build = await Bun.build({
      entrypoints: [join(playgroundRoot, "ui-playground.ts")],
      target: "browser",
      format: "esm",
      splitting: true,
      loader: {".wgsl": "text"},
    ...
    const entry = outputs.find(({name}) => name === "ui-playground.js")
    ...
    expect(await Bun.file(join(playgroundRoot, "ui-playground.ts")).text()).not.toContain("new SocketCatalogSurface")
    expect(await Bun.file(join(playgroundRoot, "ui-playground.ts")).text()).not.toContain("PlaygroundInfoSurface")
  ...
  test("publishes global and comparison readiness only after the reference texture frame", async () => {
    const client = await Bun.file(join(playgroundRoot, "ui-playground.ts")).text()
  ...
  test("keeps retained observation dev-only and routes exact browser evidence through the shared dispatcher", async () => {
    const client = await Bun.file(join(playgroundRoot, "ui-playground.ts")).text()
    const observer = await Bun.file(join(playgroundRoot, "evidence/retained-observer.ts")).text()
