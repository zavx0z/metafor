...
import {getFixtureFamily, PLAYGROUND_FIXTURES} from "./layout-fixtures.ts"
import {PLAYGROUND_POLICIES} from "./layout-policies.ts"
...
describe("dev-only nodes layout playground", () => {
  test("belongs to the centralized parent playground without a package-local server", async () => {
    const parentServer = await Bun.file(join(nodesPlaygroundRoot, "server.ts")).text()
    const manifest = await Bun.file(join(layoutRoot, "package.json")).json() as {
      scripts?: Record<string, string>
    }
    const parentManifest = await Bun.file(join(nodesPlaygroundRoot, "package.json")).json() as {
      name?: string
      scripts?: Record<string, string>
    }
  ...
  test("keeps all playground sources outside production exports and free of forbidden imports", async () => {
    ...
    expect(callers.map(({path}) => relative(playgroundRoot, path))).toEqual(["layout-policies.ts"])
    ...
    expect(adaptiveCallers.map(({path}) => relative(playgroundRoot, path))).toEqual(["layout-policies.ts"])
  })

  test("builds as a browser-only SVG tool without renderer or GPU code", async () => {
    const build = await Bun.build({
      entrypoints: [join(playgroundRoot, "layout-playground.ts")],
      target: "browser",
      format: "esm",
      minify: true,
      sourcemap: "none",
