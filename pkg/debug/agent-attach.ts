...
await runAgent(undefined, startupTarget === undefined ? {} : {startupTarget})
...
function consumeStartupTarget(rawArgs: string[]): CliStartupTarget | undefined {
  const state = globalThis as typeof globalThis & {__metaforDebugStartupTargetConsumed?: boolean}
  if (state.__metaforDebugStartupTargetConsumed === true) return undefined
  const target = startupTargetFromArgs(rawArgs)
  if (target !== undefined) state.__metaforDebugStartupTargetConsumed = true
  return target
}
