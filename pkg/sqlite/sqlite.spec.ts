...
describe("sqlite ddl", () => {
  ...
  test("держит entity-таблицы на uuid и child/subtype tables без parent-derived дублей", () => {
    ...
    const superpositionColumns = (
      db.query(`PRAGMA table_info(superposition)`).all() as Array<{ name: string }>
    ).map((row) => row.name)
    const transitionColumns = (
      db.query(`PRAGMA table_info(transition)`).all() as Array<{ name: string }>
    ).map((row) => row.name)
    const conditionColumns = (
      db.query(`PRAGMA table_info(condition)`).all() as Array<{ name: string }>
    ).map((row) => row.name)
    ...
    const processEnvColumns = (
      db.query(`PRAGMA table_info(process_env)`).all() as Array<{ name: string }>
    ).map((row) => row.name)
    const processActionColumns = (
      db.query(`PRAGMA table_info(process_action)`).all() as Array<{ name: string }>
    ).map((row) => row.name)
    const processFinallyColumns = (
      db.query(`PRAGMA table_info(process_finally)`).all() as Array<{ name: string }>
    ).map((row) => row.name)
  ...
  test("держит только structural constraints через FK UNIQUE CHECK", () => {
    ...
    expect(() =>
      db.query(`INSERT INTO field_default(field) VALUES (?)`).run("field:missing"),
    ).toThrow()
    ...
    db.query(
      ...
      .run("variant:open", "field:status", 0, "open")
    ...
    expect(() =>
      db.query(
        ...
        .run("variant:open-dup", "field:status", 0, "open-dup"),
    ).toThrow()
  ...
  test("не дублирует semantic validation триггерами и допускает structurally-valid rows", () => {
    ...
    db.query(
      ...
      .run("variant:open", "field:beta:status", 0, "open")

    db.query(
        `INSERT INTO field_enum_variant(uuid, field, position, item_value)
         VALUES (?, ?, ?, ?)`,
      )
      .run("variant:closed", "field:beta:status", 1, "closed")
    ...
    db.query(`DELETE FROM field_enum_variant WHERE uuid = ?`)
      .run("variant:closed")

    const predicateAfterEnumDelete = (
      db.query(`SELECT COUNT(*) as count FROM condition_predicate WHERE uuid = ?`).get("predicate:status:in") as { count: number }
    )
    const listItemAfterEnumDelete = (
      db.query(`SELECT COUNT(*) as count FROM condition_list_item WHERE predicate = ?`).get("predicate:status:in") as { count: number }
    )
