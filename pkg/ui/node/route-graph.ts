...
export type RoutePort = Readonly<{
  id: string
  nodeId: string
  center: FixedPoint
  side: RoutePortSide
  direction: RoutePortDirection
}>

export type RouteEdge = Readonly<{
  id: string
  sourcePortId: string
  targetPortId: string
}>
...
export type RouteSection = Readonly<{
  ...
  bendPoints: readonly FixedPoint[]
  endPoint: FixedPoint
}>

export type RouteEdgeMetrics = Readonly<{
  edgeId: string
  turns: number
  manhattan: number
  detour: number
...
export type RouteGraphResult = Readonly<{
  direction: RouteDirection
  unitsPerPixel: number
...
export type RouteGraphDiagnostic =
  ...
  | Readonly<{
    ...
    witness: Readonly<{
      ...
      rejectedTransitions: Readonly<Record<SearchRejection, number>>
...
type TerminalReservation = Readonly<{edgeId: string; portId: string; kind: "SOURCE" | "TARGET"; from: FixedPoint; to: FixedPoint}>
type RoutedSegment = Readonly<{priorEdgeId: string; from: FixedPoint; to: FixedPoint}>
type SearchTrace = {
  xs: number[]
  ys: number[]
  reachableStates: number
  reachableFrontier: unknown[]
  rejectedTransitions: Record<SearchRejection, number>
  firstRejectedHierarchyTransition: unknown | null
  pointBlocks: unknown[]
