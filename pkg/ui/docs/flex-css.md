# @metafor/ui flexCss — browser-like immediate layout
...
| Слой | Файл | Что умеет |
| --- | --- | --- |
| Low-level primitive | `src/flex.ts` (`flexRow`, `flexColumn`) | px main-axis + `"grow"` поровну, px cross-axis |
| Browser-like adapter | `src/flexCss.ts` (`flexRowCss`, `flexColumnCss`) | px, `%`, `fr` (с весами), `grow`/`auto`, padding, gap, align, justify |
...
- `pkg/ui/src/flex.ts` — низкоуровневый primitive
- `pkg/ui/src/flexCss.ts` — этот adapter
- `pkg/ui/src/flexCss.test.ts` — unit-тесты
