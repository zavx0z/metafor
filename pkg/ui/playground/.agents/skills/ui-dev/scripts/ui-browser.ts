...
type SupportedSelector = Readonly<{
  ...
  pages?: readonly PlaygroundPageDescriptor[]
...
type PlaygroundPageDescriptor = Readonly<{
  mountPath: string
  canvas: CanvasDescriptor
}>
...
function canvasForRoute(
  pages: readonly PlaygroundPageDescriptor[] | undefined,
  route: string,
  fallback: CanvasDescriptor,
): CanvasDescriptor {
  if (pages === undefined) return fallback
  const path = new URL(playgroundTargetUrl("http://playground.invalid", route)).pathname
...
async function validateRegistryRoute(url: string): Promise<void> {
  ...
  try {
    ...
    fail(`playground route preflight failed for ${url}: ${errorText(error)}`)
  }
  if (response.status >= 300 && response.status < 400) {
    const location = response.headers.get("location")
    fail(`playground route is not canonical: ${url}${location === null ? "" : `; use ${new URL(location, url).href}`}`)
  }
  if (response.status === 404) fail(`playground route is not registered: ${url}`)
  if (!response.ok) fail(`playground route preflight returned ${response.status}: ${url}`)
...
async function readDom(cdp: CdpConnection, canvasSelector: string): Promise<JsonObject> {
  return evaluate<JsonObject>(cdp, `(() => {
    ...
    const home = document.querySelector("[data-playground-home]")
    return {
      url: location.href,
