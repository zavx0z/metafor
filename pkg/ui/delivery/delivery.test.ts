...
describe("production UI delivery baseline", () => {
  ...
  test("keeps the exact MatrixInput leaf on NumberInput and independent from Field, Node and playground symbols", async () => {
    const fixture = join(uiRoot, "delivery/fixtures/exact-components-matrix-input.fixture.ts")
    const graph = (await outputSources(await buildBrowser([fixture], false)))
      .map(({source}) => source)
      .join("\n")
  ...
  test("keeps the exact ReferenceInput leaf independent from Field, Node and playground symbols", async () => {
    const fixture = join(uiRoot, "delivery/fixtures/exact-components-reference-input.fixture.ts")
    const graph = (await outputSources(await buildBrowser([fixture], false)))
      .map(({source}) => source)
      .join("\n")
    expect(graph).toContain("function ReferenceInput")
    expect(graph).not.toContain("function Field")
    expect(graph).not.toContain("@nodes/ui")
    expect(graph).not.toContain("NodeEditor")
    expect(graph).not.toContain("@ui/playground")
  ...
  test("keeps the exact EnumInput leaf independent from Field, Node and playground symbols", async () => {
    const fixture = join(uiRoot, "delivery/fixtures/exact-components-enum-input.fixture.ts")
    const graph = (await outputSources(await buildBrowser([fixture], false)))
      .map(({source}) => source)
      .join("\n")
    expect(graph).toContain("function EnumInput")
    expect(graph).not.toContain("function Field")
    expect(graph).not.toContain("@nodes/ui")
    expect(graph).not.toContain("NodeEditor")
    expect(graph).not.toContain("@ui/playground")
  ...
  test("keeps the exact CollectionInput leaf on List and Button and independent from Field, Node and playground symbols", async () => {
    const fixture = join(uiRoot, "delivery/fixtures/exact-components-collection-input.fixture.ts")
    const graph = (await outputSources(await buildBrowser([fixture], false)))
      .map(({source}) => source)
      .join("\n")
