 ...
 *
 * Matrix получает уже готовую membership/field projection и только
...
export interface PreparedEntanglementBlock {
   ...
   *
   * Это строгий контракт для matrix materialization.
   */
  fields: PreparedEntanglementField[]
}

/** Projection entanglement-блоков, готовая для materialization в Matrix. */
...
export interface PreparedEntanglementField {
  /** Индекс поля в matrix field array. */
  fieldIndex: number
