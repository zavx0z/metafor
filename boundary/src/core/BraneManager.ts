...
export class BraneManager {
  ...
  getMemoryStats(): {
    ...
    const extraAllocs = Array.from(this.branes.values()).reduce((sum, b) => sum + b.extraAllocs.length, 0)
   ...
   *
   * @returns Количество entangled блоков и их суммарный размер.
   */
  getEntangledStats(): {
    count: number
    totalSize: number
  } {
    const totalSize = Array.from(this.entangledBranes.values()).reduce((sum, b) => sum + b.blockSize, 0)
    return {
      count: this.entangledBranes.size,
      totalSize,
    }
  }
}
