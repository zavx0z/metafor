...
/**
 * E2E тесты индивидуальных суперпозиций на реальном GPU.
 *
 * Эти тесты проверяют полную интеграцию компиляции и выполнения
 * на GPU с разными superposition для каждого поля.
 *
 * ### Покрываемые сценарии:
 * 1. Поля с разными состояниями в superposition
...
describe("Boundary — E2E тесты индивидуальных суперпозиций", () => {
  ...
  describe("Поля с разными условиями перехода", () => {
    test("разные пороги для одного и того же перехода", async () => {
  ...
  describe("Поля с полностью разными машинами состояний", () => {
    test("агрессивный vs оборонительный юнит", async () => {
  ...
  describe("Обновление браны с индивидуальными superposition", () => {
    test("разные начальные значения с разными порогами", async () => {
      ...
      // Используем разные начальные значения, чтобы избежать entangled brane
      const result = await fixture.runSimulation({
        branes: {
          hp: "number",
        },
  ...
  describe("Многошаговая симуляция", () => {
    test("каждое поле проходит свой путь состояний", async () => {
