...
export class StringField extends Field {
  ...
  async default(): Promise<string | undefined> {
    const row = (
      await this.fields.wimp.sql<Array<{ default_value: string }>>`
          ...
          FROM field f
                   INNER JOIN field_string_default sd ON sd.field = f.id
  ...
  async setDefault(value: string): Promise<void> {
    ...
    await sql`DELETE FROM field_string_default WHERE field = ${id}`
    await sql`INSERT INTO field_string_default (field, default_value) VALUES (${id}, ${value})`
  }
}
