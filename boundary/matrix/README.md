...
│                                               atomic<u32>     │
│  ┌──────────────────┐  ┌──────────────────┐  ┌─────────────┐  │
│  │ bytecodeOffsets  │  │ stringRegistry   │  │ stringHeap  │  │
│  │   (Storage)      │  │   (Storage)      │  │  (Storage)  │  │
│  └──────────────────┘  └──────────────────┘  └─────────────┘  │
│                                                               │
│  ┌──────────────────────────────────────────────────────────┐ │
...
**Кратко:** Таблица указателей на heap-блоки бран.
...
```typescript
// 2 браны
braneBlockPtrs = new Uint32Array([
  1,      // brane 0: block_ptr = 1
  100,    // brane 1: block_ptr = 100
])
```

**Примечание:** Смещения `bytecode` для каждой браны хранятся отдельно в `bytecodeOffsets` на binding `5`. GPU не использует packed-структуру `[block_ptr, bytecode_offset, ...]`.
...
**Назначение:** Смещения bytecode для каждой браны. В WGSL этот буфер читается отдельно от `braneBlockPtrs`.
...
3. **Поведение флага:** Шейдер не сбрасывает `lock`. Если переход произошёл, runtime дополнительно фиксирует `heap[block_ptr + 2] = 1`, что совпадает с CPU-веткой.
