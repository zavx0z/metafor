...
То же правило относится к `index.ts` и barrel/re-export файлам. Не добавляй
re-export только ради тестов или удобства локального импорта. Если API нужен
только тесту, тест должен импортировать его относительным путём из конкретного
модуля. `index.ts` должен показывать реальную внешнюю поверхность пакета, а не
внутренние детали, открытые ради spec-файлов.
...
- Каждый домен поддерживает внутренний store: локальную проекцию и индексы
  parent-child/dependency, необходимые именно этому домену.
- Production-код не должен использовать прямые доменные API-импорты между
  ...
  отдельные Graviton/Gluon/Higgs/Photon particles через Force.
...
- Один Force impulse содержит ровно одну минимальную Particle одной сущности.
- `value` в `add` содержит только адресованную сущность, а в `replace` - только
  изменившиеся свойства этой сущности.
- Изменения применяются только инкрементально. Snapshot, `type:"create"`,
  полный rebuild и reset локальной проекции запрещены, включая cold start и
  reconnect; replay состоит из обычных поштучных particles.
...
- Production-код: прямые доменные API-imports между доменами запрещены.
- `dark/index.ts` может startup-import runtime pipeline modules только для
  загрузки локальных Force-подписок в один Bun target; это не разрешает прямые
  вызовы внутреннего API соседнего домена.
